...
-- Perbaikan:
--   - Proses potato segera setelah tuang air
--   - Teleport lebih cepat & motor tetap bergerak
--   - Card scam hanya dijalankan saat menunggu marshmallow
-- ============================================
...
-- ============================================
-- STEALTH TELEPORT - LEBIH CEPAT & MOTOR TETAP BERGERAK
-- ============================================
local function DirtBikeTeleportStealth(TargetPosition)
    ...
    if distance > 30 then
        local steps = math.min(math.floor(distance / 30), 4)
        for i = 1, steps do
...
local function ClaimAndUseCard()
    ...
    repeat
        ...
        if not AvailableATM then task.wait(0.5) end
...
local function PurchaseMarshmallowIngredients()
    ...
    task.wait(0.3)
    if Water < 1 then pcall(function() MarshRemote:FireServer("Water") end) task.wait(0.3) end
    if Gelatin < 1 then pcall(function() MarshRemote:FireServer("Gelatin") end) task.wait(0.3) end
    if SugarBlockBag < 1 then pcall(function() MarshRemote:FireServer("Sugar Block Bag") end) task.wait(0.3) end
    return true
end
        ...
        if DoorLock and Interact then
            ...
            if LockPart and KnobPrompt then
                if math.abs(LockPart.Rotation.Y) > 5 and math.abs(LockPart.Rotation.Y - 90) > 5 then
                    ...
                    repeat fireproximityprompt(KnobPrompt) task.wait(0.5) CloseAttempts = CloseAttempts+1 until math.abs(LockPart.Rotation.Y) < 5 or CloseAttempts >= 10
                    task.wait(0.3)
                end
                if LockPart.Rotation.X ~= 90 then
                    ...
                    if LockPrompt then
                        ...
                        repeat fireproximityprompt(LockPrompt) task.wait(0.3) LockAttempts = LockAttempts+1 until LockPart.Rotation.X == 90 or LockAttempts >= 10
...
local function PurchasePotatoIngredients()
    ...
    task.wait(0.3)
    if Flour < 1 then pcall(function() PotatoRemote:FireServer("Flour") end) task.wait(0.3) end
    if Potato < 1 then pcall(function() PotatoRemote:FireServer("Potato") end) task.wait(0.3) end
    return true
end
...
local function CookPotatoChips()
    ...
    for _, pot in ipairs(potList) do
        ...
        if success then
            Configuration.State.Status = "[POTATO] Pot acquired"
            return true
        end
    end
...
local function ClaimPotatoChipsAndSell()
    ...
    if AvailablePot and PotPrompt then
        for attempt = 1, 5 do
            ...
            task.wait(0.3)
            if Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Potato Chips") then
                claimed = true
                break
            end
            local notif = PlayerGui:FindFirstChild("Main") and PlayerGui.Main:FindFirstChild("BasicNotification")
            if notif and notif.TextTransparency == 0 and notif.Text:find("120 seconds") then
                Configuration.State.Status = "[POTATO] Product not ready, waiting more..."
                task.wait(10)
            end
        end
    end
...
-- ============================================
-- MAIN AUTOFARM CONTROLLER (PERBAIKAN ALUR)
-- ============================================
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- === MARSHMALLOW: Siapkan apartemen & bahan ===
        local ApartmentOk = StartMarshmallowFarm()
        if not ApartmentOk then task.wait(3) continue end

        PurchaseMarshmallowIngredients()

        -- Tuang air (timer 20 detik)
        ...
        -- === POTATO: MULAI PROSES DI LATAR BELAKANG (tidak menghalangi marshmallow) ===
        task.spawn(function()
            ...
            MixFlourAndPotato()
            CookPotatoChips()
            -- Jika gagal, tidak apa, nanti Claim akan skip
        end)

        -- === MARSHMALLOW: Tambah gula & gelatin (timer 40 detik) ===
        AddSugarAndGelatin()

        -- === CARD SCAM (opsional, berjalan selama 40 detik) ===
        ...
        -- Tunggu 40 detik marshmallow matang
        for i = 40, 1, -1 do
            if not Configuration.Main_Settings.Autofarming then break end
        ...
        -- === MARSHMALLOW: Bag dan jual ===
        BagMarshmallowAndSell()

        -- === POTATO: Claim dan feed (jika sudah matang) ===
        ClaimPotatoChipsAndSell()

        -- === CARD SCAM: Claim dan swipe jika sukses ===
        if Configuration.Main_Settings.EnableCardScam then
            ClaimAndUseCard()
        end

        -- Reset variabel potato untuk siklus berikutnya
