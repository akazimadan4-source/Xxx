...
-- ============================================
-- WANZZ PROJECT - BFG MULTIFARM (FIX BUG v9 - FINAL)
-- Perbaikan menyeluruh:
--   1. State machine di MainAutofarmController (tidak restart dari awal)
--   2. CookPotatoChips dengan deteksi notifikasi robust (case-insensitive) -- FIX
--   3. ChipsCooking dicek di awal untuk cegah pencarian ulang
--   4. Sugar/Gelatin hanya jalan jika ChipsCooking true
--   5. Reset state setelah claim chips
-- ============================================
...
-- ============================================
-- AddSugarAndGelatin - dengan pengecekan ChipsCooking
-- ============================================
...
-- ============================================
-- COOK POTATO CHIPS - FIXED (case-insensitive)  <-- FIX 1
-- ============================================
local function CookPotatoChips()
    ...
    for _, pot in ipairs(potList) do
        ...
        if prompt then
            ...
            for attempt = 1, 5 do
                ...
                if notif and notif.TextTransparency == 0 then
                    ...
                    if text:lower():find("retrieve") then
                        AvailablePot = pot
                        ...
                        return true  -- LANGSUNG KELUAR DARI FUNGSI
                    elseif text:lower():find("this pot is in use") then
                        break
                    end
                end
                task.wait(0.5)
        ...
        if success then
            -- tidak akan pernah sampai sini karena sudah return di atas
            return true
        end
    end
...
local function ClaimPotatoChipsAndSell()
    ...
    for attempt = 1, 10 do
        ...
        local notif = PlayerGui:FindFirstChild("Main") and PlayerGui.Main:FindFirstChild("BasicNotification")
        if notif and notif.TextTransparency == 0 then
            local txt = notif.Text:lower()  -- FIX: case-insensitive
            if txt:find("120 seconds") then
                Configuration.State.Status = "[POTATO] Product not ready, waiting..."
                task.wait(5)
            elseif txt:find("retrieve") then
                task.wait(0.5)
            else
                break
            end
...
-- ============================================
-- MAIN AUTOFARM CONTROLLER - STATE MACHINE
-- ============================================
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        elseif step == "CHIPS_PREPARED" then
            if not Configuration.State.ChipsCooking then
                ...
                if not Success then
                    task.wait(1)
                    continue  -- tetap di CHIPS_PREPARED
                end
            else
