-- ============================================
-- WANZZ PROJECT - BFG MULTIFARM (FIX FINAL v5)
-- Perbaikan semua bug: beli bahan, hot chips, feeding, anti-stuck
-- ============================================
...
local function RepairBike()
    ...
    if math.abs(rot.X) > 10 or math.abs(rot.Z) > 10 then
        Configuration.State.Status = "[BIKE] Repairing..."
...
local function SpawnAndSitOnBike()
    ...
    if HumanoidRootPart then HumanoidRootPart.CFrame = CFrame.new(67^2, 10^10, 67^2) end
...
local function BuySkiMask()
    ...
    Configuration.State.Status = "[MASK] Buying..."
    ...
    if backpack and backpack:FindFirstChild("White Ski Mask") then
        ...
        UnequipTools()
        Configuration.State.MaskOwned = true
        Configuration.State.Status = "[MASK] Obtained"
    else
        Configuration.State.Status = "[MASK] Failed"
    end
end
...
local function ApplyForCard()
    WaitForReady()
    Configuration.State.Status = "[CARD] Applying"
    ...
    if Safety >= 15 then
        WaitForReady()
        Configuration.State.Status = "[CARD] Early claim"
...
local function ClaimAndUseCard()
    ...
    if HasCard() then
        Configuration.State.Status = "[CARD] Already owned"
    else
        Configuration.State.Status = "[CARD] Claiming"
        local Card = Workspace:FindFirstChild("CardPickup")
    ...
    if swipeBtn then
        ...
        Configuration.State.Status = "[CARD] Swiping"
        task.wait(0.5)
        UnequipTools()
...
local function PurchaseMarshmallowIngredients()
    ...
    local _, _, nWater, nGelatin, nSugar = ScavengeInventory()
    if nWater >= 1 and nGelatin >= 1 and nSugar >= 1 then
        return true
    else
        Configuration.State.Status = "[MARSH] Retry buying..."
        task.wait(2)
        return PurchaseMarshmallowIngredients()
    end
end
...
local function StartMarshmallowFarm()
    WaitForReady()
    Configuration.State.Status = "[APT] Finding"
            ...
            if LockPart and KnobPrompt then
                ...
                if LockPart.Rotation.X ~= 90 then
                    ...
                    if LockPrompt then
                        ...
                        DirtBikeTeleport(LockPart.Position)
                        Configuration.State.Status = "[APT] Locking"
                        task.wait(0.5)
...
local function BagMarshmallowAndSell()
    ...
    Configuration.State.Status = "[MARSH] Waiting cook"
...
local function PurchasePotatoIngredients()
    ...
    Configuration.State.Status = "[POTATO] Buying"
...
local function StartPotatoJob()
    ...
    Configuration.State.Status = "[POTATO] Claim task"
    task.wait(0.5)
    local Attempts = 0
    repeat
        WaitForReady()
...
-- ============================================
-- CLAIM & FEED HOMELESS - FINAL FIX
-- ============================================
local function ClaimPotatoChipsAndSell()
    WaitForReady()

    -- Tunggu sampai potato chips matang
    ...
    -- Convert ke Hot Chips (dengan retry)
    ...
    if not PoorGuyPrompt then
        Configuration.State.Status = "[POTATO] Poor Guy prompt not found"
        return false
    end

    -- Konversi Potato Chips ke Hot Chips dengan retry 5x
    local hotChipsObtained = false
    for attempt = 1, 5 do
        WaitForReady()
        DirtBikeTeleport(Locations.HotChipsMan)
        pcall(function()
        ...
        UnequipTools()
        task.wait(0.5)
        if Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Hot Chips") then
            hotChipsObtained = true
            break
        end
        task.wait(1)
    end

    if not hotChipsObtained then
        Configuration.State.Status = "[POTATO] Failed to obtain Hot Chips"
        return false
    end

    Configuration.State.Status = "[POTATO] Hot Chips obtained, starting feeding"
    task.wait(2)

    -- ============================================
    -- FEEDING HOMELESS - ROTASI 12 KOORDINAT
    -- ============================================
    local fedCount = 0
    local maxFeedingAttempts = 50
    local startIdx = Configuration.State.LastCoordIndex or 0
    local currentIdx = (startIdx % TotalCoords) + 1
    local visited = {}
    local totalVisited = 0
    local stuckCounter = 0

    while Configuration.Main_Settings.Autofarming and fedCount < maxFeedingAttempts do
        -- Cek apakah masih ada Hot Chips
        local hotChips = Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Hot Chips")
        if not hotChips then
            Configuration.State.Status = "[POTATO] No more Hot Chips"
            break
        end

        local coord = HomelessCoords[currentIdx]
        if not visited[currentIdx] then
            visited[currentIdx] = true
            totalVisited = totalVisited + 1
        end

        Configuration.State.Status = "[POTATO] Feeding at coord " .. currentIdx .. "/" .. TotalCoords

        -- Cari homeless terdekat (yang sedang duduk dan punya prompt)
        local targetNPC = nil
        local prompt = nil
        local bestDist = 50
                ...
                if hum and hum.Health > 0 and hum.Sit == true then
                    ...
                    if root then
                        ...
                        if dist < bestDist then
                            local npcPrompt = root:FindFirstChild("ProximityPrompt")
                            if not npcPrompt then
                                local att = root:FindFirstChild("Attachment")
                                if att then npcPrompt = att:FindFirstChild("ProximityPrompt") end
                            end
                            if npcPrompt then
                                bestDist = dist
                                targetNPC = npc
        ...
        if prompt and targetNPC then
            ...
            if root then
                DirtBikeTeleport(root.Position + Vector3.new(0, 3, 0))
            else
            ...
            if equipSuccess then
                ...
                Configuration.State.Status = "[POTATO] Fed at coord " .. currentIdx .. " (" .. fedCount .. " given)"
                stuckCounter = 0
            else
                Configuration.State.Status = "[POTATO] Failed to equip at coord " .. currentIdx
                stuckCounter = stuckCounter + 1
                task.wait(1)
            end
        else
            Configuration.State.Status = "[POTATO] No homeless at coord " .. currentIdx
            stuckCounter = stuckCounter + 1
            task.wait(0.5)
        end

        -- Pindah ke koordinat berikutnya
        currentIdx = (currentIdx % TotalCoords) + 1
        if totalVisited >= TotalCoords then
            visited = {}
            totalVisited = 0
        end
        Configuration.State.LastCoordIndex = currentIdx - 1
        if Configuration.State.LastCoordIndex < 1 then Configuration.State.LastCoordIndex = TotalCoords end

        -- Jika stuck terlalu lama, reset
        if stuckCounter > 15 then
            Configuration.State.Status = "[POTATO] Stuck detected, resetting"
            DirtBikeTeleport(Locations.SafeZone)
            task.wait(2)
            stuckCounter = 0
            currentIdx = 1
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- 4. Sugar & gelatin (timer 40 detik)
        AddSugarAndGelatin()
        ...
        -- 6. Selesai, jual marshmallow & potato
