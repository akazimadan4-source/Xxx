-- ============================================
-- LENGER STORE - MULTI FARM (FIX UI STATS)
-- Perbaikan: Update label menggunakan .Text = tostring()
-- ============================================

if getgenv().LENGER_STORE_LOADED then return end
getgenv().LENGER_STORE_LOADED = true
...
-- ============================================
-- TELEPORT KE STOVE (LANGSUNG TANPA PENCARIAN ULANG)
-- ============================================
local function TeleportToStove()
    local stove = GetStoveFromApartment(Configuration.State.Apartment)
    if stove then
        DirtBikeTeleport(stove.Position)
        Configuration.State.Status = "[APT] Teleport to stove success"
        return true
    else
        Configuration.State.Status = "[APT] Stove not found, fallback to EnsureApartment"
        return EnsureApartment()
    end
end
...
-- ============================================
-- COOK POTATO CHIPS (RETURN BOOLEAN)
-- ============================================
local function CookPotatoChips()
    ...
    -- Kembali ke apartemen (prioritas langsung ke stove)
    Configuration.State.Status = "[POTATO] Pot found, teleporting to stove"
    if not TeleportToStove() then
        DirtBikeTeleport(Locations.SafeZone)
    end
    task.wait(0.5)
...
-- ============================================
-- MAIN AUTOFARM CONTROLLER (FINAL)
-- ============================================
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- 4. Pastikan posisi di stove (tanpa pencarian ulang berlebihan)
        if not TeleportToStove() then
            Configuration.State.Status = "[MARSH] Cannot teleport to stove, restarting cycle"
            task.wait(3)
            continue
        end
        ...
        -- 7. Selesaikan marshmallow & chips
...
-- ============================================
-- UI MENGGUNAKAN NEVERLOSE-STYLE
-- ============================================
...
local MainSection = MainPage:Section({Name = "Controls", Side = 1})
...
-- Section Actions
local ActionSection = MainPage:Section({Name = "Actions", Side = 2})
...
-- Halaman Stats
local StatsPage = Window:Page({Name = "Stats", Icon = "chart"})
local StatsSection = StatsPage:Section({Name = "Live Statistics", Side = 1})

-- Label-label statistik
local statusLabel = StatsSection:Label({Name = "📌 Status: Idle"})
local runtimeLabel = StatsSection:Label({Name = "⏰ Runtime: 00:00:00"})
local cashLabel = StatsSection:Label({Name = "💰 Cash: $0"})
local chipsLabel = StatsSection:Label({Name = "🍟 Chips Fed: 0"})
local marshLabel = StatsSection:Label({Name = "🧂 Marshmallows Sold: 0"})
local rejoinLabel = StatsSection:Label({Name = "🔄 Times Rejoined: 0"})
local hotChipsLabel = StatsSection:Label({Name = "🔥 Hot Chips Left: 0"})
local cyclesLabel = StatsSection:Label({Name = "🔄 Cycles: 0"})
local cardsSwipedLabel = StatsSection:Label({Name = "💳 Cards Swiped: 0"})

-- Inisialisasi window
Window:Init()

-- ============================================
-- UPDATE STATISTICS (menggunakan .Text langsung)
-- ============================================
task.spawn(function()
    ...
    local function UpdateStats()
        ...
        -- Gunakan pcall per label agar error tidak menghentikan loop
        pcall(function()
            statusLabel.Text = tostring("📌 Status: " .. (Configuration.State.Status or "Idle"))
        end)
        pcall(function()
            runtimeLabel.Text = tostring("⏰ Runtime: " .. FormatRuntime(Elapsed))
        end)
        pcall(function()
            cashLabel.Text = tostring("💰 Cash: $" .. GetCommaValue(currentCash))
        end)
        pcall(function()
            chipsLabel.Text = tostring("🍟 Chips Fed: " .. Configuration.Statistics.ChipsFed)
        end)
        pcall(function()
            marshLabel.Text = tostring("🧂 Marshmallows Sold: " .. Configuration.Statistics.MarshmallowsSold)
        end)
        pcall(function()
            rejoinLabel.Text = tostring("🔄 Times Rejoined: " .. Configuration.Statistics.TimesRejoined)
        end)
        pcall(function()
            hotChipsLabel.Text = tostring("🔥 Hot Chips Left: " .. CountHotChips())
        end)
        pcall(function()
            cyclesLabel.Text = tostring("🔄 Cycles: " .. Configuration.Statistics.CyclesCompleted)
        end)
        pcall(function()
            cardsSwipedLabel.Text = tostring("💳 Cards Swiped: " .. Configuration.Statistics.CardsSwiped)
        end)
    end
