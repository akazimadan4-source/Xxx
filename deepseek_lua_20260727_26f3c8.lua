-- ============================================
-- WANZZ PROJECT - BFG MULTIFARM (FINAL)
-- Perbaikan: Siklus stabil, UI Label (update jalan)
-- ============================================
...
local function CookPotatoChips()
    ...
    -- Kembali ke apartemen atau safe zone
...
-- Create labels using AddLabel (assuming it exists)
-- If not, fallback to AddTextBox with ReadOnly
local function SafeAddLabel(section, text)
    if section.AddLabel then
        return section:AddLabel({ Text = text })
    elseif section.AddTextBox then
        return section:AddTextBox({ Text = text, ReadOnly = true, Flag = "label_" .. tostring(#StatsSection:GetChildren()) })
    else
        -- fallback: just return a dummy with Set method
        return {
            Set = function(_, newText) end
        }
    end
end
...
local statusLabel = SafeAddLabel(StatsSection, "📌 Status: Idle")
local runtimeLabel = SafeAddLabel(StatsSection, "⏰ Runtime: 00:00:00")
local cashLabel = SafeAddLabel(StatsSection, "💰 Cash: $0")
local chipsLabel = SafeAddLabel(StatsSection, "🍟 Chips Fed: 0")
local marshLabel = SafeAddLabel(StatsSection, "🧂 Marshmallows Sold: 0")
local rejoinLabel = SafeAddLabel(StatsSection, "🔄 Times Rejoined: 0")
local hotChipsLabel = SafeAddLabel(StatsSection, "🔥 Hot Chips Left: 0")
local cyclesLabel = SafeAddLabel(StatsSection, "🔄 Cycles: 0")
local cardsSwipedLabel = SafeAddLabel(StatsSection, "💳 Cards Swiped: 0")
...
task.spawn(function()
    ...
    local function UpdateStats()
        ...
        pcall(function()
            UpdateLabel(statusLabel, "📌 Status: " .. (Configuration.State.Status or "Idle"))
            UpdateLabel(runtimeLabel, "⏰ Runtime: " .. FormatRuntime(Elapsed))
            UpdateLabel(cashLabel, "💰 Cash: $" .. GetCommaValue(currentCash))
            UpdateLabel(chipsLabel, "🍟 Chips Fed: " .. Configuration.Statistics.ChipsFed)
            UpdateLabel(marshLabel, "🧂 Marshmallows Sold: " .. Configuration.Statistics.MarshmallowsSold)
            UpdateLabel(rejoinLabel, "🔄 Times Rejoined: " .. Configuration.Statistics.TimesRejoined)
            UpdateLabel(hotChipsLabel, "🔥 Hot Chips Left: " .. CountHotChips())
            UpdateLabel(cyclesLabel, "🔄 Cycles: " .. Configuration.Statistics.CyclesCompleted)
            UpdateLabel(cardsSwipedLabel, "💳 Cards Swiped: " .. Configuration.Statistics.CardsSwiped)
        end)
    end
