-- ============================================
-- WANZZ PROJECT - BFG MULTIFARM (FIX ALL v6)
...
local function StabilizeBike()
    ...
    if vel.Magnitude > 50 or angVel.Magnitude > 5 then
        Configuration.State.Status = "[BIKE] Stabilizing..."
...
-- ============================================
-- CEK DAN PERBAIKI MOTOR YANG MENTAL
-- ============================================
local function CheckAndFixBike()
    local BikeName = string.format("%s's Car", Player.Name)
    local Bike = Workspace:FindFirstChild(BikeName)
    if not Bike then return false end
    ...
    if pos.Y < -50 or pos.Y > 500 then
        Configuration.State.Status = "[BIKE] Fixing position..."
        ...
        pcall(function()
            ...
            Bike:PivotTo(CFrame.new(Locations.SafeZone))
            task.wait(0.5)
    ...
    if math.abs(rot.X) > 45 or math.abs(rot.Z) > 45 then
        Configuration.State.Status = "[BIKE] Flipping bike..."
        ...
        pcall(function()
            ...
            Bike:PivotTo(CFrame.new(pos) * CFrame.Angles(0, rot.Y, 0))
            task.wait(0.3)
...
-- ============================================
-- TELEPORTASI HALUS (CEGAH MENTAL)
-- ============================================
local function SmoothTeleportBike(TargetPosition)
    local c = Player.Character
    ...
    local startRot = primary.Rotation
    ...
    if distance < 10 then
        pcall(function()
            ...
            Vehicle:PivotTo(CFrame.new(TargetPosition) * CFrame.Angles(0, startRot.Y, 0))
            task.wait(0.2)
    ...
    local steps = math.min(math.floor(distance / 15), 10)
    local stepSize = 1 / steps
    
    for i = 1, steps do
        local fraction = i * stepSize
        ...
        pcall(function()
            ...
            Vehicle:PivotTo(CFrame.new(midPos) * CFrame.Angles(0, startRot.Y, 0))
            task.wait(0.05)
...
local DirtBikeTeleport = SmoothTeleportBike
...
local function FindFeedableHomeless(coord)
    ...
    local npcFolder = Workspace:FindFirstChild("Folders")
    if npcFolder then
        npcFolder = npcFolder:FindFirstChild("NPCs")
    end
...
local function ClaimPotatoChipsAndSell()
    ...
    while Configuration.Main_Settings.Autofarming and fedCount < maxFeedingAttempts do
        ...
        -- Cari homeless yang bisa diberi makan di koordinat ini
        local targetNPC, rootPart, prompt = FindFeedableHomeless(coord)

        -- Jika tidak ada, cari homeless lain di mana saja
        ...
        if targetNPC and rootPart and prompt then
            ...
            if equipSuccess then
                ...
                for attempt = 1, 3 do
                    ...
                    -- Cek apakah hot chips berkurang
                    local newChips = Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Hot Chips")
                    if newChips then
                        -- Hitung ulang
                        local oldCount = hotChips and 1 or 0
                        local newCount = newChips and 1 or 0
                        if newCount > 0 then
        ...
        -- Jika terlalu banyak gagal, reset dan tunggu
...
if getgenv().AutoRejoinerEnabled then
    ...
    Configuration.Statistics.TimesRejoined = Configuration.