-- ============================================
-- LENGER MULTI FARM (FULL FIXED)
-- Perbaikan: RPC fallback, DirtBikeTeleport robust,
-- Spawn & sitting lebih stabil.
-- ============================================
...
if not RPC then
    error("Tidak ditemukan remote untuk spawn bike. Coba restart game atau laporkan.")
end
...
-- ============================================
-- SPAWN & SIT BIKE (dengan pengecekan lebih ketat)
-- ============================================
local function SpawnAndSitOnBike()
    ...
    -- Tunggu sebentar agar bike stabil
    task.wait(0.5)

    local DriveSeat = Bike:FindFirstChild("DriveSeat")
    if not DriveSeat then
        Configuration.State.Status = "[BIKE] DriveSeat not found"
        return false
    end
    ...
    task.wait(1.5) -- tambah waktu untuk duduk

    -- Verifikasi apakah benar-benar duduk
    local h = GetHumanoid()
    if h and h.SeatPart and h.SeatPart.Parent == Bike then
        Configuration.State.BikeSitting = true
        Configuration.State.BikeSpawned = true
        ...
        Configuration.State.RespawnPending = false
        Configuration.State.Status = "[BIKE] Failed to sit"
        return false
    end
end

-- ============================================
-- DIRTBIKE TELEPORT (ROBUST)
-- ============================================
local function DirtBikeTeleport(TargetPosition)
    ...
    -- Jika tidak duduk, coba duduk ulang
    ...
    -- Lepaskan semua anchor sementara
    local anchoredParts = {}
    for _, part in pairs(Vehicle:GetDescendants()) do
        if part:IsA("BasePart") then
            if part.Anchored then
                anchoredParts[part] = true
                part.Anchored = false
            end
        end
    end

    -- Teleport dengan beberapa percobaan
    local success = false
    for attempt = 1, 3 do
        pcall(function()
        ...
        task.wait(0.1)
        if primary and (primary.Position - TargetPosition).Magnitude < 5 then
            success = true
            break
        end
    end
...
local function BuySkiMask()
    ...
    if not DirtBikeTeleport(Locations.SkiMask) then return end
    task.wait(0.5)
...
local function PurchaseFakeID()
    ...
    repeat WaitForReady() if not DirtBikeTeleport(Locations.FakeID) then return false end task.wait(0.05)
    ...
    repeat
        WaitForReady()
        if not DirtBikeTeleport(Locations.FakeID) then return false end
...
local function ApplyForCard()
    ...
    repeat if not DirtBikeTeleport(Locations.ApplyForCard) then return false end task.wait(0.05)
    ...
    repeat
        WaitForReady()
        if not DirtBikeTeleport(Locations.ApplyForCard) then return false end
    ...
    if Safety >= 15 then
        ...
        if Card then
            ...
            if CardPrompt then
                for _ = 1, 10 do
                    if not DirtBikeTeleport(Card.Position) then return false end
...
local function ClaimAndUseCard()
    ...
    repeat
        WaitForReady()
        if not DirtBikeTeleport(AvailableATM.Position) then return false end
...
local function PurchaseMarshmallowIngredients()
    ...
    if not DirtBikeTeleport(Locations.BuyMarsh) then return false end
        ...
        if Board then
            ...
            if backboard then
                ...
                if Prompt then
                    ...
                    WaitForReady()
                    if not DirtBikeTeleport(backboard.Position) then return false end
        ...
        if DoorLock and Interact then
            ...
            if LockPart and KnobPrompt then
                if math.abs(LockPart.Rotation.Y) > 5 and math.abs(LockPart.Rotation.Y - 90) > 5 then
                    ...
                    if not DirtBikeTeleport(LockPart.Position) then return false end
...
local function PourWater()
    ...
    if not DirtBikeTeleport(Stove.Position) then return false end
...
local function BagMarshmallowAndSell()
    ...
    if not DirtBikeTeleport(Locations.SafeZone) then return false end
    local waitTime = 0
    ...
    repeat WaitForReady() if not DirtBikeTeleport(Locations.BuyMarsh) then return false end task.wait(0.05) lamontAttempts = lamontAttempts + 1 until Workspace:FindFirstChild("Folders") and Workspace.Folders:FindFirstChild("NPCs") and Workspace.Folders.NPCs:FindFirstChild("Lamont Bell") or lamontAttempts > 20
...
local function PurchasePotatoIngredients()
    ...
    if not DirtBikeTeleport(Locations.BuyPotato) then return false end
...
local function StartPotatoJob()
    ...
    if not DirtBikeTeleport(Locations.Clipboard) then return false end
...
local function CutPotato()
    ...
    repeat
        WaitForReady()
        if not DirtBikeTeleport(Locations.PotatoCutter) then return false end
...
local function BagPotato()
    ...
    repeat
        WaitForReady()
        if not DirtBikeTeleport(Locations.PlasticBagLab) then return false end
        task.wait(0.25)
...
local function MixFlourAndPotato()
    ...
    repeat
        WaitForReady()
        if not DirtBikeTeleport(Locations.FlourBowl) then return false end
        task.wait(0.25)
...
local function CookPotatoChips()
    ...
    for _, Object in next, Pots:GetChildren() do
        ...
        if Object:IsA("UnionOperation") then
            ...
            repeat
                WaitForReady()
                if not DirtBikeTeleport(Object.Position) then return false end
...
local function FeedHomelessAtCoord(coord)
    ...
    if not DirtBikeTeleport(coord) then return false end
    task.wait(1.2)
    ...
    if promptPart and promptPart:IsA("BasePart") then
        if not DirtBikeTeleport(promptPart.Position + Vector3.new(0, 1, 0)) then return false end
        task.wait(0.5)
    else
        if not DirtBikeTeleport(coord) then return false end
        task.wait(0.5)
...
local function ClaimPotatoChipsAndSell()
    ...
    repeat
        WaitForReady()
        if not DirtBikeTeleport(AvailablePot.Position) then return false end
    ...
    repeat
        WaitForReady()
        if not DirtBikeTeleport(Locations.HotChipsMan) then return false end
        task.wait(0.05)
...
Library:Notify({
    Title = "LENGER MULTI FARM",
    Lifetime = 3,
    Text = "Script loaded! Remote: " .. RPC.Name
})