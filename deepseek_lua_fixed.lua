...
-- ============================================
-- STEALTH TELEPORT (SLOW + VARIATIVE)
local function DirtBikeTeleportStealth(TargetPosition)
    local vehicle = workspace:FindFirstChild("DirtBike") or workspace:FindFirstChildOfClass("Model")
    ...
    -- step count: 1 step per 4 studs, minimum 15
    local steps = math.max(15, math.floor(distance / 4))

    -- anchor only primary part
    primary.Anchored = true

    for i = 1, steps do
        local alpha = i / steps
        local basePos = startPos:Lerp(TargetPosition, alpha)
        ...
        primary.CFrame = CFrame.new(basePos + offset)

        -- random delay 0.12 - 0.28
        task.wait(math.random(12, 28) / 100)
    end

    primary.CFrame = CFrame.new(TargetPosition)
