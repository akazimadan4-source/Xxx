...
-- Perbaikan: Siklus stabil, feeding seperti penjual
-- ============================================
...
-- ============================================
-- COOK POTATO CHIPS (RESET DAN KELUAR)
-- ============================================
...
-- ============================================
-- FUNGSI FEED HOMELESS (DISEDERHANAKAN SEPERTI INTERAKSI PENJUAL)
-- ============================================
local function FeedHomelessAtCoord(coord)
    ...
    -- Cari ProximityPrompt terdekat (tanpa filter, seperti cari penjual)
    ...
    -- Teleport sedikit lebih dekat ke part prompt (jika bisa)
    ...
    -- Equip Hot Chips (seperti equip marshmallow)
    ...
    -- Fire prompt sekali (seperti interaksi penjual)
    fireproximityprompt(bestPrompt)
    task.wait(0.5)
    UnequipTools()

    -- Anggap sukses jika Hot Chips berkurang
    task.wait(0.5)
...
local function ClaimPotatoChipsAndSell()
    ...
    -- RESET variabel pot sebelum digunakan
    if not AvailablePot or not PotTimer then
        Configuration.State.Status = "[POTATO] No pot available, skipping..."
        return false
    end
    ...
    -- Feeding homeless dengan pendekatan baru (seperti interaksi penjual)
    local fedCount = 0
