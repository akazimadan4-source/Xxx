...
-- Perbaikan: Teleport halus, feeding akurat (hanya homeless duduk),
--            motor tidak terpental, semua 12 titik dikunjungi.
-- ============================================
...
-- ============================================
-- CLAIM & FEED HOMELESS - DIPERBAIKI (HANYA HOMELESS DUDUK)
-- ============================================
local function ClaimPotatoChipsAndSell()
    ...
    -- ============================================
    -- FEEDING HOMELESS - PERBAIKAN: HANYA YANG DUDUK, SEMUA 12 TITIK
    -- ============================================
    local fedCount = 0
    ...
    while Configuration.Main_Settings.Autofarming and fedCount < maxFeedingAttempts do
        ...
        -- Tentukan koordinat berikutnya (urutan siklik)
        ...
        -- Cari NPC homeless yang sedang duduk (Sit == true)
        local targetNPC = nil
        local prompt = nil

        -- 1. Cari di Folder NPCs dulu
        ...
        if npcFolder then
            for _, npc in pairs(npcFolder:GetChildren()) do
                if npc:IsA("Model") then
                    ...
                    if hum and upper and hum.Sit == true then
        ...
        -- 2. Jika tidak ketemu, cari di seluruh Workspace
        ...
        if prompt then
            ...
            if equipSuccess then
                ...
                Configuration.State.Status = "[POTATO] Fed sitting homeless at " .. nextIdx .. " (" .. fedCount .. " given)"
            else
                Configuration.State.Status = "[POTATO] Failed equip at " .. nextIdx
                task.wait(0.5)
            end
        else
            -- Tidak ada homeless duduk di koordinat ini, skip
            Configuration.State.LastCoordIndex = nextIdx
            Configuration.State.Status = "[POTATO] No sitting homeless at " .. nextIdx .. ", skipping"
            task.wait(0.3)
        end
    end
