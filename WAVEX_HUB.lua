...
if isfunctionhooked then
    if isfunctionhooked(FireServer) or isfunctionhooked(UnreliableFireServer) or isfunctionhooked(InvokeServer) and LPH_OBFUSCATED then
        return Services.LocalPlayer:Kick("WAVEX HUB | Security : You are running another script, please disable it and execute again")
    end
end
...
local window = library:window({name = "WAVEX", suffix = " HUB", gameInfo = string.format("WAVEX HUB : %s", Game_Name:lower())})
