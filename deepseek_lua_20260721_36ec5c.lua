...
-- Menggunakan 5 koordinat homeless pilihan
-- ============================================
...
-- ============================================
-- DAFTAR KOORDINAT HOMELESS (5 TITIK PILIHAN)
-- ============================================
local HomelessCoords = {
    Vector3.new(-315.35, 3.72, -361.56),   -- titik 1
    Vector3.new(-273.52, 3.85, -211.32),   -- titik 2
    Vector3.new(52.89, 3.72, -425.36),     -- titik 4
    Vector3.new(65.12, 3.73, 68.10),       -- titik 7
    Vector3.new(26.04, 3.73, 217.89),      -- titik 8
}
local TotalCoords = #HomelessCoords  -- = 5
...
-- ============================================
-- CLAIM & FEED HOMELESS (DIPERBAIKI - HANYA 5 TITIK)
-- ============================================
local function ClaimPotatoChipsAndSell()
    ...
    -- ============================================
    -- FEEDING HOMELESS - PERBAIKAN: HANYA 5 TITIK
    -- ============================================
    local fedCount = 0
    local maxFeedingAttempts = 200
    ...
    while Configuration.Main_Settings.Autofarming and fedCount < maxFeedingAttempts do
        ...
        if not hotChips then
            Configuration.State.Status = "[POTATO] No more Hot Chips, stopping feed"
            break
        end
        ...
        if not h or not h.SeatPart then
            Configuration.State.Status = "[POTATO] Bike missing, re-spawning..."
            SpawnAndSitOnBike()
            task.wait(1)
            h = GetHumanoid()
            if not h or not h.SeatPart then
                Configuration.State.Status = "[POTATO] Can't get bike, retry later"
                task.wait(3)
                continue
            end
        end
        ...
        Configuration.State.Status = "[POTATO] Trying coord " .. nextIdx .. "/" .. TotalCoords

        -- CARI NPC HOMELESS
        local targetNPC = nil
        local prompt = nil

        local function findNPC(list)
            for _, npc in pairs(list) do
                if npc:IsA("Model") then
                    local hum = npc:FindFirstChildOfClass("Humanoid")
                    local upper = npc:FindFirstChild("UpperTorso")
                    if hum and upper then
                        local dist = (upper.Position - coord).Magnitude
                        if dist < 60 then
        ...
        if npcFolder then
            local found, sitting = findNPC(npcFolder:GetChildren())
            if found then targetNPC = found end
        end

        if not targetNPC then
            local found, sitting = findNPC(Workspace:GetDescendants())
            if found then targetNPC = found end
        end
        ...
        if not targetNPC then
            ...
            local upper = targetNPC:FindFirstChild("UpperTorso")
            if upper then
                prompt = upper:FindFirstChild("ProximityPrompt")
                if not prompt then
        ...
        if prompt then
            ...
            for attempt = 1, 3 do
                teleportSuccess = DirtBikeTeleport(coord)
                if teleportSuccess then break end
                task.wait(0.5)
                Configuration.State.Status = "[POTATO] Teleport retry " .. attempt
            end

            if not teleportSuccess then
                Configuration.State.Status = "[POTATO] Teleport failed at " .. nextIdx
                task.wait(1)
            ...
            if equipSuccess then
                ...
                Configuration.State.LastCoordIndex = nextIdx
                Configuration.State.Status = "[POTATO] Fed at " .. nextIdx .. " (" .. fedCount .. " given)"
            else
                Configuration.State.Status = "[POTATO] Failed equip at " .. nextIdx
                task.wait(0.5)
            end
        else
            Configuration.State.LastCoordIndex = nextIdx
            Configuration.State.Status = "[POTATO] No prompt at " .. nextIdx .. ", skipping"
            task.wait(0.3)
        end
    end
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- 6. Sekarang marshmallow selesai, potato chips juga selesai (120 detik)
