...
-- ============================================
-- STEALTH TELEPORT (KECEPATAN LAMBAT - TETAP)
-- ============================================
local function DirtBikeTeleportStealth(TargetPosition)
    ...
    if distance > 30 then
        -- KECEPATAN LAMBAT (sesuai permintaan)
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- ===== MODIFIKASI: Tunggu hingga proses potato chips selesai mengambil pot (maks 5 detik) =====
        local waitCount = 0
        while not Configuration.State.WaitingForChips and waitCount < 5 do
            task.wait(1)
            waitCount = waitCount + 1
            Configuration.State.Status = "[MARSH] Waiting for potato chips to settle... (" .. waitCount .. "/5)"
        end
        -- Setelah itu, baru lanjut ke AddSugarAndGelatin tanpa konflik teleport
