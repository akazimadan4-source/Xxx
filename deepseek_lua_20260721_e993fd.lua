...
-- Perbaikan:
--   - Timer pot lebih fleksibel (0, 00, atau hilang)
--   - Claim keripik cek Backpack + Character
--   - UI Statistik pakai Paragraph (biar muncul)
--   - Reset AvailablePot & PotPrompt setelah claim
-- ============================================
...
-- ============================================
-- POTATO CHIPS FUNCTIONS (DIPERBAIKI TOTAL)
-- ============================================
...
local function ClaimPotatoChipsAndSell()
    ...
    -- ===== TUNGGU SAMPAI TIMER 0 ATAU HILANG =====
    ...
    while waited < maxWait do
        ...
        -- Cek apakah timer masih ada dan masih di game
        local timerExists = PotTimer and PotTimer.Parent and PotTimer:IsDescendantOf(game)
        if timerExists then
            ...
            -- Timer hilang, anggap sudah selesai
            timerText = ""
            break
        end
    ...
    if AvailablePot and PotPrompt then
        for attempt = 1, 8 do  -- tambah percobaan
            WaitForReady()
            ...
            -- Cek notifikasi kalau masih belum siap
            local notif = PlayerGui:FindFirstChild("Main") and PlayerGui.Main:FindFirstChild("BasicNotification")
    ...
    -- Reset pot setelah berhasil claim
    AvailablePot = nil
    PotPrompt = nil
    PotTimer = nil
        ...
        if nearestNPC then
            ...
            if upper then
                ...
                if prompt then
                    ...
                    -- NPC tidak punya prompt, hapus dari daftar
            ...
            -- Tidak ada NPC dekat, teleport ke koordinat terdekat
            local bestCoord = nil
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- Pastikan reset setelah siklus (walaupun sudah di-reset di Claim)
...
-- ============================================
-- UI MENGGUNAKAN LILUI (DIPERBAIKI PAKAI PARAGRAPH)
-- ============================================
...
-- ============================================
-- UPDATE STATISTICS (pakai SetDesc untuk Paragraph)
-- ============================================
task.spawn(function()
    ...
    local function UpdateStats()
        ...
        pcall(function()
            statusLabel:SetDesc("📌 Status: " .. (Configuration.State.Status or "Idle"))
            runtimeLabel:SetDesc("⏰ Runtime: " .. FormatRuntime(Elapsed))
            cashLabel:SetDesc("💰 Cash: $" .. GetCommaValue(currentCash))
            chipsLabel:SetDesc("🍟 Chips Fed: " .. Configuration.Statistics.ChipsFed)
            marshLabel:SetDesc("🧂 Marshmallows Sold: " .. Configuration.Statistics.MarshmallowsSold)
            rejoinLabel:SetDesc("🔄 Times Rejoined: " .. Configuration.Statistics.TimesRejoined)
            hotChipsLabel:SetDesc("🔥 Hot Chips Left: " .. CountHotChips())
            cyclesLabel:SetDesc("🔄 Cycles: " .. Configuration.Statistics.CyclesCompleted)
            cardsSwipedLabel:SetDesc("💳 Cards Swiped: " .. Configuration.Statistics.CardsSwiped)
        end)
    end
