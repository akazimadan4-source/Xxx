-- ============================================
-- WANZZ PROJECT - MARSHMALLOW FARM (ONLY)
-- Siklus: Bike → Apartment → Water (20s) → Sugar+Gelatin (40s) → Bag & Sell
-- UI: Venyx Library
-- ============================================
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- 6. Tunggu 40 detik (marshmallow masak)
        for i = 40, 1, -1 do
            if not Configuration.Main_Settings.Autofarming then break end
...
-- UI VENYX
-- ============================================
local Venyx = loadstring(game:HttpGet("https://raw.githubusercontent.com/violin-suzutsuki/Venyx/main/source.lua"))()
local Window = Venyx:new("WANZZ MARSHMALLOW")

-- ============================================
-- MAIN PAGE
-- ============================================
local MainPage = Window:addPage("Main", "home")
local MainSection = MainPage:addSection("Controls")

local autofarmToggle = MainSection:addToggle("Autofarming", false, function(value)
...
local antiDeathToggle = MainSection:addToggle("Anti Death", true, function(value)
    Configuration.Main_Settings.AutoAntiDeath = value
end)

local rejoinToggle = MainSection:addToggle("Auto Rejoiner", true, function(value)
    Configuration.Main_Settings.AutoRejoiner = value
end)

MainSection:addButton("Spawn DirtBike ($35K)", function()
    pcall(function()
        RPC:FireServer(makeBuffer("\001"), "Purchase", "DirtBike")
    end)
end)

MainSection:addButton("Rejoin Game", function()
    DoRejoin()
end)
...
-- STATS PAGE
-- ============================================
local StatsPage = Window:addPage("Stats", "chart")
local StatsSection = StatsPage:addSection("Live Statistics")

-- Gunakan addTextbox sebagai label statistik
local statusBox = StatsSection:addTextbox("Status", "Idle", function() end)
local runtimeBox = StatsSection:addTextbox("Runtime", "00:00:00", function() end)
local cashBox = StatsSection:addTextbox("Cash", "$0", function() end)
local marshBox = StatsSection:addTextbox("Marshmallows Sold", "0", function() end)
local rejoinBox = StatsSection:addTextbox("Times Rejoined", "0", function() end)
local cyclesBox = StatsSection:addTextbox("Cycles", "0", function() end)
...
task.spawn(function()
    ...
    local function UpdateStats()
        ...
        pcall(function()
            StatsSection:updateTextbox(statusBox, "Status", "📌 " .. (Configuration.State.Status or "Idle"))
            StatsSection:updateTextbox(runtimeBox, "Runtime", "⏰ " .. FormatRuntime(Elapsed))
            StatsSection:updateTextbox(cashBox, "Cash", "💰 $" .. GetCommaValue(currentCash))
            StatsSection:updateTextbox(marshBox, "Marshmallows Sold", "🧂 " .. Configuration.Statistics.MarshmallowsSold)
            StatsSection:updateTextbox(rejoinBox, "Times Rejoined", "🔄 " .. Configuration.Statistics.TimesRejoined)
            StatsSection:updateTextbox(cyclesBox, "Cycles", "🔄 " .. Configuration.Statistics.CyclesCompleted)
        end)
    end
