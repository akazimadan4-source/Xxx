-- ============================================
-- LENGER MULTI FARM (FIX TELEPORT TURUN MOTOR)
-- Perbaikan: Teleport bertahap agar tidak turun.
-- ============================================
...
-- ============================================
-- FUNGSI CARI RPC DENGAN BRUTE FORCE
-- ============================================
...
if not RPC then
    error("Tidak ditemukan remote untuk spawn bike.")
end
...
local function RepairBike()
    ...
    if math.abs(rot.X) > 10 or math.abs(rot.Z) > 10 then
        pcall(function()
            Bike:PivotTo(CFrame.new(primary.Position) * CFrame.Angles(0, rot.Y, 0))
...
-- ============================================
-- SPAWN & SIT BIKE (dengan stabilisasi)
-- ============================================
...
-- ============================================
-- DIRTBIKE TELEPORT (STEALTH - TIDAK TURUN MOTOR)
-- ============================================
local function DirtBikeTeleport(TargetPosition)
    ...
    -- Teleport bertahap jika jarak jauh
    if distance > 30 then
    ...
    pcall(function()
        ...
        task.wait(0.3)
        -- Unanchor sementara agar fisik aktif
        for _, part in pairs(Vehicle:GetDescendants()) do
            if part:IsA("BasePart") then
                part.Anchored = false
            end
...
Library:Notify({
    Title = "LENGER MULTI FARM",
    Lifetime = 3,
    Text = "Script loaded! Teleport fixed."
})