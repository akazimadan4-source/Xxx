-- ============================================
-- LENGER MULTI FARM (FIX RPC & ADAPTIF)
-- Support update terbaru, fallback robust
-- ============================================
...
-- ============================================
-- FUNGSI CARI RPC DENGAN FALLBACK (ADAPTIF)
-- ============================================
local function findRPC()
    -- Tunggu game benar-benar siap
    repeat task.wait(0.5) until game:IsLoaded() and Player and Player.Character

    -- Coba cari RemoteEvents di ReplicatedStorage
    local RemoteEvents = ReplicatedStorage:FindFirstChild("RemoteEvents")
    if not RemoteEvents then
        -- Coba tunggu 15 detik
        RemoteEvents = ReplicatedStorage:FindFirstChild("RemoteEvents") or ReplicatedStorage:WaitForChild("RemoteEvents", 15)
    end

    if not RemoteEvents then
        -- Jika masih tidak ada, cari folder yang namanya mengandung "Remote"
        for _, child in pairs(ReplicatedStorage:GetChildren()) do
            if child:IsA("Folder") and (child.Name:lower():find("remote") or child.Name == "RemoteEvents") then
                RemoteEvents = child
                break
            end
        end
    ...
    if not RemoteEvents then
        -- Fallback terakhir: cari secara rekursif semua anak di ReplicatedStorage
        local function findRemoteEvents(obj)
            for _, child in pairs(obj:GetChildren()) do
                if child:IsA("Folder") and (child.Name:lower():find("remote") or child.Name == "RemoteEvents") then
                    return child
                end
                local found = findRemoteEvents(child)
                if found then return found end
            end
            return nil
        end
        RemoteEvents = findRemoteEvents(ReplicatedStorage)
    end

    if not RemoteEvents then
        error("RemoteEvents not found. Pastikan game sudah dimuat.")
    end

    -- Cari RPC di dalam RemoteEvents
    local RPC = RemoteEvents:FindFirstChild("RPC")
    if not RPC then
        RPC = RemoteEvents:FindFirstChild("RPC") or RemoteEvents:WaitForChild("RPC", 15)
    end

    if not RPC then
        -- Cari alternatif nama
        for _, child in pairs(RemoteEvents:GetChildren()) do
            if child.Name == "RPC" or child.Name:lower():find("rpc") then
                RPC = child
                break
            end
        end
    ...
    if not RPC then
        error("RPC not found. Coba restart game.")
    end
...
-- ============================================
-- SPAWN & SIT BIKE (MULTI FARM VERSION)
-- ============================================
...
-- ============================================
-- DIRTBIKE TELEPORT (MULTI FARM STYLE)
-- ============================================
...
local function BuySkiMask()
    ...
    local StoreRemote = ReplicatedStorage:FindFirstChild("RemoteEvents") and ReplicatedStorage.RemoteEvents:FindFirstChild("StorePurchase")
...
local function PurchaseMarshmallowIngredients()
    ...
    local MarshRemote = ReplicatedStorage:FindFirstChild("RemoteEvents") and ReplicatedStorage.RemoteEvents:FindFirstChild("StorePurchase")
...
local function PurchasePotatoIngredients()
    ...
    local PotatoRemote = ReplicatedStorage:FindFirstChild("RemoteEvents") and ReplicatedStorage.RemoteEvents:FindFirstChild("StorePurchase")
...
Library:Notify({
    Title = "LENGER MULTI FARM",
    Lifetime = 3,
    Text = "Script loaded successfully! (Adaptif RPC)"
})