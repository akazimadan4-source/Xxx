-- ============================================
-- WANZZ PROJECT - BFG MULTIFARM (FIX BUG)
-- Perbaikan: Teleport halus, feeding akurat, motor tidak terpental
-- ============================================
...
-- ============================================
-- FUNGSI REPAIR BIKE (jika terbalik/rusak)
-- ============================================
...
-- ============================================
-- STEALTH TELEPORT (DIPERBAIKI AGAR MOTOR TIDAK TERPENTAL)
-- ============================================
local function DirtBikeTeleportStealth(TargetPosition)
    ...
    if distance > 30 then
        local steps = math.min(math.floor(distance / 20), 8) -- lebih halus
        for i = 1, steps do
    ...
    pcall(function()
        ...
        -- Tunggu sebentar lalu lepas anchoring
        task.wait(0.3)
        for _, part in pairs(Vehicle:GetDescendants()) do
            if part:IsA("BasePart") then
                part.Anchored = false
        ...
        task.wait(0.2)
        -- Kunci lagi sebentar agar tidak terpental
        for _, part in pairs(Vehicle:GetDescendants()) do
            if part:IsA("BasePart") then
                part.Anchored = true
            end
    ...
    -- Cek apakah motor masih miring, perbaiki jika perlu
    RepairBike()
    return true
end
...
-- ============================================
-- FUNGSI FEED HOMELESS (DIPERBAIKI)
-- ============================================
local function FeedHomelessAtCoord(coord)
    -- Parameter: coord = Vector3 tujuan (salah satu dari HomelessCoords)
    if not coord then return false end

    -- 1. Cek apakah masih ada Hot Chips di backpack
    ...
    -- 2. Teleport ke koordinat dengan motor (sudah pakai DirtBikeTeleport)
    Configuration.State.Status = "[FEED] Teleporting to " .. tostring(coord)
    DirtBikeTeleport(coord)
    task.wait(1.2) -- beri waktu stabil

    -- 3. Cari ProximityPrompt terdekat di sekitar koordinat (radius 30)
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("ProximityPrompt") then
            -- Dapatkan part induk (bisa langsung parent atau parent adalah Attachment)
            local parentPart = obj.Parent
            if parentPart and parentPart:IsA("BasePart") then
                local dist = (parentPart.Position - coord).Magnitude
                if dist < 30 and dist < bestDist then
                    bestDist = dist
                    bestPrompt = obj
                    promptPart = parentPart
                end
            else
                -- Coba cek jika parent adalah Attachment
    ...
    -- Jika tidak ada prompt, coba cari NPC Homeless (model dengan Humanoid dan Sit==true)
            ...
            if model:IsA("Model") then
                ...
                if hum and hum.Sit == true then
                    ...
                    if root then
                        ...
                        if dist < 30 and dist < bestDist then
                            -- Cari prompt di root atau attachment
    ...
    -- 5. Teleport lebih dekat ke promptPart (jika ada)
    if promptPart then
    ...
    -- 6. Equip Hot Chips dengan retry
    ...
    -- 7. Fire prompt beberapa kali untuk memastikan
    local fireAttempts = 0
    local maxFire = 3
    repeat
        fireproximityprompt(bestPrompt)
        task.wait(0.2)
        fireAttempts = fireAttempts + 1
    until fireAttempts >= maxFire

    -- 8. Tunggu sebentar, lalu lepas equip
    task.wait(0.5)
    UnequipTools()

    -- 9. Verifikasi apakah Hot Chips berkurang (artinya feeding berhasil)
    task.wait(0.5)
    local remaining = Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Hot Chips")
    if not remaining then
        -- Hot Chips habis = feeding sukses
        Configuration.State.Status = "[FEED] Success, Hot Chips consumed"
        return true
    else
        -- Anggap berhasil jika prompt sudah difire dan tidak ada error.
...
-- ============================================
-- CLAIM & FEED HOMELESS - DIPERBAIKI
-- ============================================
local function ClaimPotatoChipsAndSell()
    ...
    -- ============================================
    -- FEEDING HOMELESS (LOOP DIPERBAIKI)
    -- ============================================
    local fedCount = 0
    ...
    while Configuration.Main_Settings.Autofarming and fedCount < maxFeedingAttempts do
        ...
        -- Pilih koordinat berikutnya (cycle)
        ...
        -- Panggil fungsi feeding yang baru
        ...
        if success then
            ...
            -- Jika gagal berturut-turut, skip ke koordinat berikutnya
        ...
        -- Jeda antar feeding agar tidak terlalu cepat
        task.wait(0.5)
    end
