...
-- Perbaikan: 
--   - Randomisasi pemilihan pot agar tidak stack
--   - Deteksi homeless dengan radius lebih besar dan cari NPC terdekat
--   - Teleport langsung ke NPC, bukan ke koordinat
-- ============================================
...
-- ============================================
-- POTATO CHIPS FUNCTIONS (DIPERBAIKI)
-- ============================================
...
-- ============================================
-- COOK POTATO CHIPS - DIPERBAIKI DENGAN RANDOMISASI
-- ============================================
local function CookPotatoChips()
    ...
    -- Acak urutan pot agar tidak semua bot memilih pot yang sama
    for i = #potList, 2, -1 do
        local j = Random:NextInteger(1, i)
        potList[i], potList[j] = potList[j], potList[i]
    end

    -- Coba setiap pot secara berurutan
            ...
            for attempt = 1, 5 do
                ...
                if notif and notif.TextTransparency == 0 then
                    ...
                    elseif text == "This pot is in use." then
                        -- Pot dipakai, coba pot berikutnya
                        break
                    end
                end
                task.wait(0.5)
...
local function ClaimPotatoChipsAndSell()
    ...
    if npcFolder then
        for _, npc in pairs(npcFolder:GetChildren()) do
            if npc:IsA("Model") and npc:FindFirstChildOfClass("Humanoid") then
                ...
                if upper then
                    -- Cek apakah NPC ini adalah homeless (bisa dicek dari nama atau posisi dekat koordinat)
    ...
    -- Jika tidak ada NPC di folder, coba cari di seluruh Workspace
    ...
    -- Loop feeding dengan NPC terdekat
    while Configuration.Main_Settings.Autofarming and fedCount < maxFeedingAttempts do
        ...
        -- Cari NPC terdekat dari posisi player
        local char = Player.Character
        ...
        for _, npc in pairs(allHomelessNPCs) do
            ...
            if upper then
                ...
                if dist < nearestDist and dist < 150 then -- radius 150
                    nearestDist = dist
                    nearestNPC = npc
                end
            end
        ...
        if nearestNPC then
            ...
            if upper then
                ...
                if prompt then
                    ...
                    -- Teleport ke NPC, bukan ke koordinat
                    ...
                    -- NPC tidak punya prompt, tandai sebagai tidak valid
            ...
            -- Tidak ada NPC dalam radius, coba teleport ke koordinat terdekat dari daftar
            local bestCoord = nil
