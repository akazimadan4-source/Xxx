--[[
    AUTO FARM - FIXED
    - Tidak perlu Empty Bag di awal
    - Cukup Water, Sugar, Gelatin
    - Setelah farming selesai, hasil dijual otomatis
]]
...
-- ============================================
-- UI CREATION (sama seperti sebelumnya)
-- ============================================
...
-- ============================================
-- FARMING SYSTEM (tanpa cek Empty Bag)
-- ============================================
...
-- ============================================
-- MAIN PROCESS (FIXED - tanpa cek Empty Bag)
-- ============================================
...
local function DoFarmingAndSell()
    ...
    task.spawn(function()
        -- 1. CEK BAHAN (WATER, SUGAR, GELATIN SAJA)
        statusLabel.Text = "Status: checking materials..."
        if not HasTool("Water") then
            statusLabel.Text = "Status: Water not found!"
        ...
        if not HasTool("Sugar Block Bag") then
            statusLabel.Text = "Status: Sugar not found!"
            statusLabel.TextColor3 = colors.red
            isRunning = false
            startBtn.Text = "START FARMING"
            startBtn.BackgroundColor3 = colors.green
        ...
        if not HasTool("Gelatin") then
            statusLabel.Text = "Status: Gelatin not found!"
            statusLabel.TextColor3 = colors.red
            isRunning = false
            startBtn.Text = "START FARMING"
            startBtn.BackgroundColor3 = colors.green
        ...
        statusLabel.Text = "Status: materials OK!"
        task.wait(1)
        ...
        -- 3. FARMING LOOP (tanpa Empty Bag)
        local farming = true
        ...
        farmTask = task.spawn(function()
            while farming and isRunning do
                ...
                -- CEK BAHAN MASIH ADA?
                if not HasTool("Water") and not HasTool("Sugar Block Bag") and not HasTool("Gelatin") then
        ...
        -- Tunggu farming selesai (atau sampai bahan habis)
        while farming and isRunning do
            task.wait(1)
            if not HasTool("Water") and not HasTool("Sugar Block Bag") and not HasTool("Gelatin") then
                farming = false
...
print("✅ Auto Farm FIXED loaded!")
print("📌 Cukup siapkan Water, Sugar Block Bag, dan Gelatin.")
print("📌 Empty Bag akan muncul setelah proses memasak.")
print("📌 Klik START FARMING untuk mulai.")