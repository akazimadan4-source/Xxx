-- ============================================
-- LENGER MULTI FARM (STABLE SEQUENTIAL - FINAL)
-- Semua workflow sequential, validasi ketat, 
-- tidak ada task.spawn di autofarm.
-- ============================================
...
-- Mendapatkan teks task update
local function GetTaskText()
    local taskUpdate = PlayerGui:FindFirstChild("Main") and PlayerGui.Main:FindFirstChild("TaskUpdate")
    if taskUpdate then
        local label = taskUpdate:FindFirstChild("TextLabel")
        if label then return label.Text end
    end
    return ""
end

-- Menunggu task update mengandung pola tertentu
local function WaitForTask(pattern, timeout)
    timeout = timeout or 30
    local start = os.clock()
    repeat
        task.wait(0.5)
        local text = GetTaskText()
...
-- ============================================
-- 2. DIRTBIKE TELEPORT (dengan retry & validasi)
-- ============================================
local function DirtBikeTeleport(TargetPosition, maxRetries)
...
-- ============================================
-- 4. MARSHMALLOW WORKFLOW (sequential dengan validasi)
-- ============================================
local function PurchaseMarshmallowIngredients()
    ...
    local _, _, W2, G2, S2 = ScavengeInventory()
    return (W2 >= 1 and G2 >= 1 and S2 >= 1)
end
...
local function PourWater()
    ...
    repeat
        ...
        Safety = Safety + 1
        if Safety >= 10 then
            Configuration.State.Status = "[MARSH] Water failed"
            return false
        end
...
local function AddSugarAndGelatin()
    ...
    repeat
        ...
        UnequipTools()
        Safety = Safety + 1
        if Safety >= 10 then return false end
...
-- ============================================
-- 5. POTATO CHIPS WORKFLOW (dengan task update)
-- ============================================
...
local function StartPotatoJob()
    ...
    until GetTaskText():match("Task:") or GetTaskText():match("began")
    return true
end
...
local function CutPotato()
    ...
    -- Tunggu task update berubah ke langkah berikutnya (misal "Bag" atau "Head")
    WaitForTask("Bag|Head", 10)
    return true
end
...
local function BagPotato()
    ...
    until GetTaskText():match("Head") or GetTaskText():match("Mix")
    return true
end
...
local function MixFlourAndPotato()
    ...
    task.wait(3.5)
    -- Tunggu task update ke "Cook" atau "Pot"
    WaitForTask("Cook|Pot", 10)
    return true
end
...
local function CookPotatoChips()
    ...
    -- Tunggu task update ke "Claim" atau "Retrieve"
    WaitForTask("Retrieve|Claim|seconds", 10)
    return true
end
...
local function ClaimPotatoChipsAndSell()
    ...
    repeat
        WaitForReady()
        if not DirtBikeTeleport(foundPot.Position) then return false end
    ...
    -- FEED HOMELESS (sequential sampai habis)
    ...
    while HasItem("Hot Chips") and Configuration.Main_Settings.Autofarming do
        ...
        for _, HomelessRef in next, availableHomeless do
            ...
            local UpperTorso = HomelessRef:FindFirstChild("UpperTorso")
            if not UpperTorso then continue end
            local Prompt = UpperTorso:FindFirstChild("ProximityPrompt")
            if not Prompt then continue end

            if not DirtBikeTeleport(UpperTorso.Position) then continue end
...
local function BuySkiMask()
    ...
    if not DirtBikeTeleport(Locations.SkiMask) then return false end
    task.wait(0.5)
...
local function cekDarah(char)
    ...
    hum.HealthChanged:Connect(function(hp)
        ...
        if hp > 0 and hp < 95 then
            ...
            DirtBikeTeleport(Locations.Healing, 5)
        end
    end)
end
...
-- ============================================
-- MAIN AUTOFARM CONTROLLER (SEQUENTIAL, no task.spawn)
-- ============================================
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- 8. Wait for marshmallow to cook (45s)
        for i = 45, 1, -1 do
