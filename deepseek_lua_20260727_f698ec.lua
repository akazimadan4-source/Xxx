-- ============================================
-- LENGER MULTI FARM (MODIFIED WITH MULTI FARM LOGIC)
-- Perbaikan: Status live, UI title, Stove handling
-- ============================================
...
-- ============================================
-- VARIABEL GLOBAL UNTUK PROSES (MULTI FARM STYLE)
-- ============================================
...
-- ============================================
-- FUNGSI MULTI FARM (DIREPLIKASI DARI MULTI FARM.TXT)
-- ============================================
...
-- 2. DIRTBIKE TELEPORT (VERSI MULTI FARM)
local function DirtBikeTeleport(TargetPosition)
    ...
    if not h.SeatPart then
        Configuration.State.Status = "[BIKE] Not on bike, re-sitting..."
...
-- 4. START MARSHMALLOW FARM (APARTMENT SETUP)
...
local function PurchaseMarshmallowIngredients()
    ...
    DirtBikeTeleport(Vector3.new(512.820, 4.000, 595.580)) -- BuyMarsh
...
local function PourWater()
    ...
    CookPrompt = Stove:FindFirstChild("Attachment") and Stove.Attachment:FindFirstChild("ProximityPrompt")
    StoveTimer = Stove:FindFirstChild("Timer") and Stove.Timer:FindFirstChild("TextLabel")
...
local function BagMarshmallowAndSell()
    ...
    DirtBikeTeleport(Vector3.new(-478.840, 24.000, 389.200)) -- SafeZone
    repeat task.wait(1) until StoveTimer and StoveTimer.Text == "0"
    ...
    repeat WaitForReady() DirtBikeTeleport(Vector3.new(512.820, 4.000, 595.580)) task.wait(0.05) lamontAttempts = lamontAttempts + 1 until Workspace:FindFirstChild("Folders") and Workspace.Folders:FindFirstChild("NPCs") and Workspace.Folders.NPCs:FindFirstChild("Lamont Bell") or lamontAttempts > 20
    ...
    for _, Object in next, backpack:GetChildren() do
        if tostring(Object):find("Marshmallow") then
            WaitForReady()
            DirtBikeTeleport(Vector3.new(512.820, 4.000, 595.580))
            EquipTool(Object)
            task.wait(0.5)
...
local function PurchasePotatoIngredients()
    ...
    DirtBikeTeleport(Vector3.new(-759.920, -0.025, -195.870)) -- BuyPotato
    task.wait(0.5)
...
local function StartPotatoJob()
    ...
    DirtBikeTeleport(Vector3.new(-479.230, 5.342, -433.270)) -- Clipboard
    Configuration.State.Status = "[POTATO] Claiming task"
    task.wait(0.5)
    local Attempts = 0
    repeat
        WaitForReady()
        DirtBikeTeleport(Vector3.new(-479.230, 5.342, -433.270))
        task.wait(0.25)
...
local function CutPotato()
    ...
    repeat
        WaitForReady()
        DirtBikeTeleport(Vector3.new(-456.320, 1.870, -466.840)) -- PotatoCutter
...
local function BagPotato()
    ...
    repeat
        WaitForReady()
        DirtBikeTeleport(Vector3.new(-456.280, 1.654, -472.670)) -- PlasticBagLab
        task.wait(0.25)
...
local function MixFlourAndPotato()
    ...
    repeat
        WaitForReady()
        DirtBikeTeleport(Vector3.new(-494.640, 1.579, -518.580)) -- FlourBowl
        task.wait(0.25)
...
local function CookPotatoChips()
        ...
        if Object:IsA("UnionOperation") then
            ...
            if Notif then
                ...
                elseif notifText == "You have 120 seconds to retrieve your product out of the pot when its done." then
                    ...
                    PotTimer = Object:FindFirstChild("Timer") and Object.Timer:FindFirstChild("TextLabel")
                end
            end
        end
    end
    ...
    if EnsureApartment() then
        ...
        if stove then
            ...
            DirtBikeTeleport(Vector3.new(-478.840, 24.000, 389.200))
        end
    else
        DirtBikeTeleport(Vector3.new(-478.840, 24.000, 389.200))
    end
...
local function FindAvailableHomeless()
    ...
    if not homelessFolder then return Available end
...
-- 9. CLAIM POTATO CHIPS AND SELL (with Homeless feeding)
local function ClaimPotatoChipsAndSell()
    ...
    repeat
        WaitForReady()
        DirtBikeTeleport(Vector3.new(-41.000, 3.000, -25.000)) -- HotChipsMan
        task.wait(0.05)
    ...
    repeat
        WaitForReady()
        DirtBikeTeleport(Vector3.new(-41.000, 3.000, -25.000))
    ...
    for _, HomelessRef in next, AvailableHomeless do
        if not Player:FindFirstChild("Backpack") or not Player.Backpack:FindFirstChild("Hot Chips") then break end
        local HomelessName = tostring(HomelessRef)
        repeat
            WaitForReady()
            DirtBikeTeleport(HomelessRef:FindFirstChild("UpperTorso").Position)
            task.wait(0.05)
...
-- 10. FAKE ID & CARD SCAM WORKFLOW
local function PurchaseFakeID()
    WaitForReady()
    Configuration.State.Status = "[CARD] Buying fake ID"
    repeat WaitForReady() DirtBikeTeleport(Vector3.new(214.960, 1.857, -332.330)) task.wait(0.05)
    ...
    repeat
        WaitForReady()
        DirtBikeTeleport(Vector3.new(214.960, 1.857, -332.330))
...
local function ApplyForCard()
    ...
    repeat DirtBikeTeleport(Vector3.new(-49.210, 4.000, -310.810)) task.wait(0.05)
    ...
    repeat
        WaitForReady()
        DirtBikeTeleport(Vector3.new(-49.210, 4.000, -310.810))
...
-- 11. BUY SKI MASK (DIPERLUKAN UNTUK FAKE ID)
local function BuySkiMask()
    ...
    DirtBikeTeleport(Vector3.new(-366.980, 0.528, -320.630)) -- SkiMask
    task.wait(0.5)
...
local function cekDarah(char)
    ...
    hum.HealthChanged:Connect(function(hp)
        ...
        if hp > 0 and hp < 95 then
            ...
            DirtBikeTeleport(Vector3.new(-769.000, 6.000, 654.000)) -- Healing
        end
    end)
end
...
-- ============================================
-- MAIN AUTOFARM CONTROLLER (MULTI FARM WORKFLOW)
-- ============================================
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- Potato preparation (run in parallel with water waiting)
        ...
        task.spawn(function()
            ...
            BagPotato()
            MixFlourAndPotato()
            potFound = CookPotatoChips()
            potatoPrepDone = true
        end)

        -- Wait for water to cook a bit before adding sugar/gelatin
        for i = 20, 1, -1 do
            if not Configuration.Main_Settings.Autofarming then break end
        ...
        -- Ensure apartment valid before adding sugar
        if not EnsureApartment() then
