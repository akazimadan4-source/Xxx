...
-- Perbaikan: Siklus stabil, feeding seperti penjual, UI Paragraph
-- ============================================
...
local function AddSugarAndGelatin()
    ...
    if not Stove then
        Configuration.State.Status = "[MARSH] Stove not found, re-finding apartment..."
        -- Coba cari apartemen lagi
        if not StartMarshmallowFarm() then
            return false
        end
        AptObj = Configuration.State.Apartment
        if not AptObj then return false end
        -- Ulangi pencarian stove
        if tostring(AptObj):match("Home") then
            Stove = AptObj:FindFirstChild("Cooking Pot")
        else
            local Interior = AptObj:FindFirstChild("Interior")
        ...
        if not Stove then
            Configuration.State.Status = "[MARSH] Stove still not found"
            return false
        end
    end
    ...
    -- Pastikan kita teleport ke stove
    DirtBikeTeleport(Stove.Position)
    task.wait(0.5)
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- ================================
        -- 4. Pastikan apartemen masih valid sebelum tambah gula
        -- ================================
        if not EnsureApartment() then
...
-- ============================================
-- UI MENGGUNAKAN LILUI (DENGAN PARAGRAPH)
-- ============================================
