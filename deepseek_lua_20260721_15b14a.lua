...
-- ============================================
-- STEALTH TELEPORT (DIPERLAMBAT SEDIKIT & AMAN)
-- ============================================
local function DirtBikeTeleportStealth(TargetPosition)
    ...
    if distance > 30 then
        local steps = math.min(math.floor(distance / 18), 9)
        for i = 1, steps do
...
local function PurchaseMarshmallowIngredients()
    ...
    -- Verifikasi ulang
    local _, _, w, g, s = ScavengeInventory()
    if w < 1 or g < 1 or s < 1 then
        Configuration.State.Status = "[MARSH] Failed to buy all ingredients"
        return false
    end
    return true
end
...
local function AddSugarAndGelatin()
    ...
    -- Pastikan ada sugar dan gelatin
    ...
    if not sugar or not gelatin then
        Configuration.State.Status = "[MARSH] Missing ingredients, buying..."
        PurchaseMarshmallowIngredients()
        sugar = Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Sugar Block Bag")
        gelatin = Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Gelatin")
        if not sugar or not gelatin then
            Configuration.State.Status = "[MARSH] Still missing ingredients, skipping"
            return false
        end
    end
    ...
    local addedSugar = false
    for attempt = 1, 5 do
        WaitForReady()
        DirtBikeTeleport(Stove.Position)
        local sugarItem = Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Sugar Block Bag")
        if sugarItem then EquipTool(sugarItem) end
        task.wait(0.5)
        if CookPrompt then fireproximityprompt(CookPrompt) end
        task.wait(1)
        local notif = PlayerGui:FindFirstChild("Main") and PlayerGui.Main:FindFirstChild("BasicNotification")
        if notif and notif.TextTransparency == 0 then
            if notif.Text:find("added") or notif.Text:find("Sugar") then
                addedSugar = true
                break
            end
        end
    ...
    if not addedSugar then
        Configuration.State.Status = "[MARSH] Failed to add sugar"
        return false
    end

    -- Tambahkan gelatin
    Configuration.State.Status = "[MARSH] Adding gelatin"
    local addedGelatin = false
    for attempt = 1, 5 do
        WaitForReady()
        DirtBikeTeleport(Stove.Position)
        local gelatinItem = Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Gelatin")
        if gelatinItem then EquipTool(gelatinItem) end
        task.wait(0.5)
        if CookPrompt then fireproximityprompt(CookPrompt) end
        task.wait(1)
        local notif = PlayerGui:FindFirstChild("Main") and PlayerGui.Main:FindFirstChild("BasicNotification")
        if notif and notif.TextTransparency == 0 then
            if notif.Text:find("added") or notif.Text:find("Gelatin") then
                addedGelatin = true
                break
            end
        end
    ...
    if not addedGelatin then
        Configuration.State.Status = "[MARSH] Failed to add gelatin"
        return false
    end
...
local function PurchasePotatoIngredients()
    ...
    local p, f = ScavengeInventory()
    if p < 1 or f < 1 then
        Configuration.State.Status = "[POTATO] Failed to buy ingredients"
        return false
    end
    return true
end
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        local sugarOk = AddSugarAndGelatin()
        if not sugarOk then
            Configuration.State.Status = "[MARSH] Failed to add sugar/gelatin, retry next cycle"
            -- tetap lanjut, tapi mungkin akan skip card scam? Biarkan saja
        end
