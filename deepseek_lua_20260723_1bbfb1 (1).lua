...
local AimMode = "PC"
local espCache = {}
local AimFriendsOnly = false   -- true = hanya teman, false = hanya non-teman
local Friends = {}             -- daftar player yang dianggap teman (key = player, value = true)
local ShowPlayerList = false   -- tampilkan daftar player?
...
-- ESP SETTINGS (NAME & SKELETON ONLY)
local espSet = {
    Enabled = false,
    ShowName = true,
    ShowSkeletons = true,
...
Players.PlayerAdded:Connect(function(p)
    if p ~= Player then
        createEsp(p)
        -- tambahkan ke list player nanti
    end
end)
...
-- SILENT AIM (dengan filter teman)
    ...
    for _,v in pairs(Players:GetPlayers()) do
        ...
        if v ~= Player and char then
            ...
            if root and hum and hum.Health > 0 then
                -- Filter berdasarkan teman/non-teman
                local isFriend = Friends[v] == true
                if AimFriendsOnly then
                    if not isFriend then continue end  -- hanya teman
                else
                    if isFriend then continue end      -- hanya non-teman
                end
...
-- ========================================
-- LENGER HUB DENGAN LIST PLAYER
-- ========================================

-- Background Particles (sama seperti sebelumnya)
...
Panel.Size = UDim2.new(0, 380, 0, 430) -- lebih tinggi untuk list
Panel.Position = UDim2.new(0.5, -190, 0.25, 0)
...
-- TOMBOL BARU: Aim Teman & Player List
local AimFriendBtn = Btn("AIM TEMAN: OFF", 0.05, 0.79, Color3.fromRGB(80,20,50))
local ListBtn = Btn("LIST PLAYER", 0.52, 0.79, Color3.fromRGB(20,40,80))
...
ListFrame.Position = UDim2.new(0.05, 0, 0.92, 0)
ListFrame.BackgroundColor3 = Color3.fromRGB(5,5,20)
...
Instance.new("UIStroke", ListFrame).Color = Color3.fromRGB(100,50,255)

-- Fungsi untuk membuat checkbox player
local function CreatePlayerCheckbox(player)
    if player == Player then return end
    local row = Instance.new("Frame", ListFrame)
    row.Size = UDim2.new(1,0,0,22)
    row.BackgroundTransparency = 1
    row.LayoutOrder = #ListFrame:GetChildren() + 1

    local check = Instance.new("TextButton", row)
    check.Size = UDim2.new(0,20,0,20)
    check.Position = UDim2.new(0,0,0,1)
    check.BackgroundColor3 = Friends[player] and Color3.fromRGB(0,200,100) or Color3.fromRGB(50,50,50)
    check.BackgroundTransparency = 0.2
    check.Text = Friends[player] and "✓" or ""
    ...
    check.MouseButton1Click:Connect(function()
        Friends[player] = not Friends[player]
        check.BackgroundColor3 = Friends[player] and Color3.fromRGB(0,200,100) or Color3.fromRGB(50,50,50)
        check.Text = Friends[player] and "✓" or ""
...
local function UpdateListCanvas()
    local count = #ListFrame:GetChildren()
    ListFrame.CanvasSize = UDim2.new(0,0,0,count * 22)
end

-- Inisialisasi list untuk semua player
local function RefreshPlayerList()
    -- Hapus semua kecuali mungkin nanti kita rebuild
    for _, child in ipairs(ListFrame:GetChildren()) do
        child:Destroy()
    end
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= Player then
            CreatePlayerCheckbox(p)
        end
    end
    UpdateListCanvas()
end

-- Saat player bergabung/keluar, refresh list
Players.PlayerAdded:Connect(function(p)
    if p ~= Player then
        -- tambahkan ke list jika list sedang terbuka, atau nanti saat refresh
        -- kita refresh seluruh list setiap kali ada perubahan agar simpel
        RefreshPlayerList()
    end
end)
...
-- Tombol Aim Teman
AimFriendBtn.MouseButton1Click:Connect(function()
    AimFriendsOnly = not AimFriendsOnly
    AimFriendBtn.Text = AimFriendsOnly and "AIM TEMAN: ON" or "AIM TEMAN: OFF"
    AimFriendBtn.BackgroundColor3 = AimFriendsOnly and Color3.fromRGB(0,200,100) or Color3.fromRGB(80,20,50)
end)

-- Tombol List Player
ListBtn.MouseButton1Click:Connect(function()
    ShowPlayerList = not ShowPlayerList
    ListFrame.Visible = ShowPlayerList
    ListBtn.BackgroundColor3 = ShowPlayerList and Color3.fromRGB(0,200,100) or Color3.fromRGB(20,40,80)
    if ShowPlayerList then
        RefreshPlayerList() -- refresh saat dibuka
    end
end)
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    Panel.Size = isMin and UDim2.new(0,380,0,40) or UDim2.new(0,380,0,430)
