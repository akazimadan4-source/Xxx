-- ============================================
-- WANZZ PROJECT - BFG MULTIFARM (FIX BUG v3)
-- Perbaikan: Tidak stack di pot potato, card scam berjalan
-- ============================================
...
local function RepairBike()
    ...
    local isUpsideDown = rot.X > 80 or rot.X < -80 or rot.Z > 80 or rot.Z < -80
    local isFloating = pos.Y > 100 or pos.Y < -50
    
    if isUpsideDown or isFloating or math.abs(rot.X) > 15 or math.abs(rot.Z) > 15 then
        ...
        local targetPos = Vector3.new(pos.X, 3.5, pos.Z)
        local newCF = CFrame.new(targetPos) * CFrame.Angles(0, rot.Y, 0)
        ...
        Configuration.State.Status = "[BIKE] Repaired"
        return true
    end
    return false
end
...
local function DirtBikeTeleportStealth(TargetPosition)
    ...
    if distance > 20 then
        local steps = math.min(math.floor(distance / 15), 12)
        for i = 1, steps do
...
-- ============================================
-- PERBESAR PROXIMITY PROMPT
-- ============================================
local function EnlargePrompt(prompt)
...
local function PurchaseFakeID()
    ...
    for attempt = 1, maxAttempts do
        ...
        -- Cari NPC
        local FakeIDSeller = Workspace:FindFirstChild("Folders") and 
                            Workspace.Folders:FindFirstChild("NPCs") and 
                            Workspace.Folders.NPCs:FindFirstChild("FakeIDSeller")
        if not FakeIDSeller then
            task.wait(0.5)
            continue
        end
        
        local BuyIDPrompt = FakeIDSeller:FindFirstChild("UpperTorso") and 
                           FakeIDSeller.UpperTorso:FindFirstChild("Attachment") and 
                           FakeIDSeller.UpperTorso.Attachment:FindFirstChild("ProximityPrompt")
        if BuyIDPrompt then 
            EnlargePrompt(BuyIDPrompt)
            -- Equip ski mask
            local SkiMask = Player.Character and Player.Character:FindFirstChild("White Ski Mask") or 
                           (Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("White Ski Mask"))
        ...
        -- Cek apakah dapat fake ID
        if Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Fake ID") then
            Configuration.State.Status = "[CARD] Fake ID obtained"
            return true
        end
        task.wait(1)
    end
    Configuration.State.Status = "[CARD] Failed to get Fake ID"
    return false
end
...
local function ApplyForCard()
    ...
    for attempt = 1, maxAttempts do
        ...
        local BankTeller = Workspace:FindFirstChild("Folders") and 
                          Workspace.Folders:FindFirstChild("NPCs") and 
                          Workspace.Folders.NPCs:FindFirstChild("Bank Teller")
        if not BankTeller then
            task.wait(0.5)
            continue
        end
        
        local BankPrompt = BankTeller:FindFirstChild("UpperTorso") and 
                          BankTeller.UpperTorso:FindFirstChild("Attachment") and 
                          BankTeller.UpperTorso.Attachment:FindFirstChild("ProximityPrompt")
        if BankPrompt then 
            EnlargePrompt(BankPrompt)
        ...
        if notif and notif.TextTransparency == 0 then
            if notif.Text:match("successful") or notif.Text:match("approved") then
                Configuration.State.Status = "[CARD] Application approved"
                return true
            end
        end
...
local function ClaimAndUseCard()
    ...
    if HasCard() then
        ...
        -- Tunggu card muncul di workspace
        local Card = nil
        for i = 1, 20 do
            Card = Workspace:FindFirstChild("CardPickup")
            if Card then break end
            task.wait(1)
        end
        
        if not Card then 
            Configuration.State.Status = "[CARD] Card not found"
            return false 
        end
        
        local CardPrompt = Card:FindFirstChild("Attachment") and Card.Attachment:FindFirstChild("ProximityPrompt")
        if CardPrompt then EnlargePrompt(CardPrompt) end
    ...
    for attempt = 1, 15 do
        ...
        if AvailableATM then break end
        task.wait(1)
    end
    ...
    if ATMPrompt then EnlargePrompt(ATMPrompt) end
    ...
    for attempt = 1, 10 do
        ...
        task.wait(0.5)
        if PlayerGui:FindFirstChild("ATM") then break end
        task.wait(0.5)
    end
...
local function PourWater()
    ...
    if CookPrompt then 
        CookPrompt = CookPrompt:FindFirstChild("ProximityPrompt")
        EnlargePrompt(CookPrompt)
    end
    ...
    return true
endlocal function AddSugarAndGelatin()
    WaitForReady()
    local AptObj = Configuration.State.Apartment
    if not AptObj then return false end
    local Stove
...
local function BagMarshmallowAndSell()
    ...
    if LamontPrompt then 
        LamontPrompt = LamontPrompt:FindFirstChild("ProximityPrompt")
        EnlargePrompt(LamontPrompt)
    end
    UnequipTools()
    local backpack = Player:FindFirstChild("Backpack")
    ...
    Configuration.State.MarshDone = true
    return true
end

-- ============================================
-- POTATO CHIPS FUNCTIONS (FIX: TIDAK STACK)
-- ============================================
...
local function StartPotatoJob()
    ...
    if ClipboardPrompt then 
        EnlargePrompt(ClipboardPrompt)
        pcall(function() ClipboardPrompt.MaxActivationDistance = 9e9 end) 
    end
...
local function MixFlourAndPotato()
    ...
    if BowlPrompt then 
        EnlargePrompt(BowlPrompt)
        pcall(function() BowlPrompt.MaxActivationDistance = 9e9 end) 
    end
...
-- ============================================
-- COOK POTATO - FIX: TIDAK STACK, LANGSUNG CLAIM
-- ============================================
local function CookPotatoChips()
    ...
    for _, Object in next, Pots:GetChildren() do
        if potFound then break end
        if Object:IsA("UnionOperation") then
            -- Cek apakah pot kosong
            local timerObj = Object:FindFirstChild("Timer")
            local isInUse = false
            ...
            for attempt = 1, 5 do
                ...
                if Notif then
                    ...
                    if text == "You have 120 seconds to retrieve your product out of the pot when its done." then
                        -- Pot sudah berisi produk, langsung claim
                        AvailablePot = Object
                        PotPrompt = Object:FindFirstChild("ProximityPrompt")
                        if PotPrompt then EnlargePrompt(PotPrompt) end
                        if timerObj then PotTimer = timerObj:FindFirstChild("TextLabel") end
                        potFound = true
                        Configuration.State.Status = "[POTATO] Found ready pot"
                        return true
                ...
                -- Jika tidak ada notif, coba cook
                if not isInUse then
                    local hasFlour = Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Flour")
                    local hasPotato = Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Potato")
                    
                    if hasFlour and hasPotato then
                        AvailablePot = Object
                        PotPrompt = Object:FindFirstChild("ProximityPrompt")
                        if PotPrompt then EnlargePrompt(PotPrompt) end
                        if timerObj then PotTimer = timerObj:FindFirstChild("TextLabel") end
    ...
    -- Jika tidak ada pot kosong, coba cari pot yang sudah selesai
    if not potFound then
        for _, Object in next, Pots:GetChildren() do
            if Object:IsA("UnionOperation") then
                local timerObj = Object:FindFirstChild("Timer")
                if timerObj then
                    local timerLabel = timerObj:FindFirstChild("TextLabel")
                    if timerLabel and timerLabel.Text == "0" then
                        AvailablePot = Object
                        PotPrompt = Object:FindFirstChild("ProximityPrompt")
                        if PotPrompt then EnlargePrompt(PotPrompt) end
                        PotTimer = timerLabel
                        potFound = true
                        Configuration.State.Status = "[POTATO] Found finished pot"
                        return true
                    end
                end
            end
...
-- ============================================
-- CLAIM & FEED HOMELESS - FIX: LANGSUNG CLAIM
-- ============================================
local function ClaimPotatoChipsAndSell()
    WaitForReady()
    
    -- Jika pot sudah siap, langsung claim
    if AvailablePot and PotPrompt then
        ...
        repeat
            ...
            if claimAttempts > 15 then break end
        until Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Potato Chips")
    end
    
    -- Tunggu jika belum ada chips
    local waitTime = 0
    while waitTime < 30 do
        if Player:FindFirstChild("Backpack") and Player.Backpack:FindFirstChild("Potato Chips") then break end
        task.wait(1)
        waitTime = waitTime + 1
        Configuration.State.Status = "[POTATO] Waiting for chips... " .. waitTime .. "s"
    end
    ...
    repeat
        ...
        if convertAttempts > 15 then break end
    ...
    if not PoorGuy then 
        Configuration.State.Status = "[POTATO] Poor Guy not found"
        return false 
    end
    ...
    repeat
        ...
        if hotChipsAttempts > 15 then break end
    ...
    while Configuration.Main_Settings.Autofarming and fedCount < maxFeedingAttempts do
        ...
        if targetNPC then
            local root = targetNPC:FindFirstChild("HumanoidRootPart") or targetNPC:FindFirstChild("UpperTorso")
            if root then
                prompt = root:FindFirstChild("ProximityPrompt")
                if not prompt then
            ...
            for _, obj in pairs(Workspace:GetDescendants()) do
                if obj:IsA("ProximityPrompt") then
                    ...
                    if parentPart and parentPart:IsA("BasePart") then
                        local distance = (parentPart.Position - coord).Magnitude
                        if distance < 25 then
                            prompt = obj
                            break
                        end
                    ...
                    if att and att:IsA("Attachment") then
                        ...
                        if part and part:IsA("BasePart") then
                            local distance = (part.Position - coord).Magnitude
                            if distance < 25 then
                                prompt = obj
                                break
                            end
        ...
        if prompt then
            ...
            Configuration.State.LastCoordIndex = nextIdx
            Configuration.State.Status = "[POTATO] No prompt at coord " .. nextIdx .. ", skipping"
            task.wait(0.5)
        end
    end

    Configuration.State.Status = "[POTATO] Feeding done (" .. fedCount .. " chips given)"
    Configuration.State.PotatoDone = true
    return true
end
...
-- ============================================
-- MAIN AUTOFARM CONTROLLER - FIX: TIDAK STACK
-- ============================================
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- Reset state per siklus
        Configuration.State.PotatoDone = false
        Configuration.State.MarshDone = false

        -- ================================
        -- 1. Siapkan apartemen dan bahan marshmallow
        -- ================================
        ...
        -- ================================
        -- 2. Tuang air (mulai timer 20 detik)
        -- ================================
        local WaterOk = PourWater()
        if not WaterOk then
            repeat StartMarshmallowFarm(); WaterOk = PourWater() until WaterOk or not Configuration.Main_Settings.Autofarming
        end

        -- ================================
        -- 3. Sambil menunggu 20 detik, lakukan persiapan potato chips
        -- ================================
        task.spawn(function()
        ...
        -- ================================
        -- 4. Tambahkan sugar dan gelatin (mulai timer 40 detik)
        -- ================================
        AddSugarAndGelatin()

        -- ================================
        -- 5. Selama 40 detik, lakukan card scam (jika aktif) - FIX: LANGSUNG JALAN
        -- ================================
        local cardScamDone = false
        if Configuration.Main_Settings.EnableCardScam then
            task.spawn(function()
                -- Card scam sequence
                local fakeIDSuccess = PurchaseFakeID()
                if fakeIDSuccess then
                    local applySuccess = ApplyForCard()
                    if applySuccess then
                        -- Tunggu beberapa detik sebelum claim card
                        for i = 30, 1, -1 do
                            if not Configuration.Main_Settings.Autofarming then break end
                            Configuration.State.Status = "[CARD] Waiting for approval: " .. i .. "s"
                            task.wait(1)
                        end
                        ClaimAndUseCard()
                        cardScamDone = true
        ...
        -- Countdown 40 detik untuk marshmallow matang
        for i = 40, 1, -1 do
            if not Configuration.Main_Settings.Autofarming then break end
        ...
        -- ================================
        -- 6. Selesaikan marshmallow
        -- ================================
        BagMarshmallowAndSell()

        -- ================================
        -- 7. Selesaikan potato chips - FIX: LANGSUNG CLAIM TANPA STUCK
        -- ================================
        -- Tunggu potato selesai jika belum
        local potatoWait = 0
        while not Configuration.State.PotatoDone and potatoWait < 60 do
            task.wait(1)
            potatoWait = potatoWait + 1
            Configuration.State.Status = "[POTATO] Waiting for cook... " .. potatoWait .. "s"
        end
        ...
        -- ================================
        -- 8. Card scam final check
        -- ================================
        if Configuration.Main_Settings.EnableCardScam and not cardScamDone then
            -- Coba sekali lagi
            PurchaseFakeID()
            ApplyForCard()
            for i = 20, 1, -1 do
                if not Configuration.Main_Settings.Autofarming then break end
                Configuration.State.Status = "[CARD] Final wait: " .. i .. "s"
                task.wait(1)
            end
            ClaimAndUseCard()
        end

        -- Reset variabel untuk siklus berikutnya
...
local cardsSwipedLabel = StatusTab:Label({ Title = "💳 Cards Swiped: 0" })
