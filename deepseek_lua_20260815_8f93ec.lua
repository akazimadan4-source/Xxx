...
-- ============================================
-- AUTO RESPAWN (LANGSUNG KE APARTEMEN)
-- ============================================
local function AutoRespawnToApartment()
    local character = Player.Character
    if not character then
        Player.CharacterAdded:Wait()
        character = Player.Character
    ...
    if humanoid then
        ...
        -- Langsung teleport ke apartemen
        if Configuration.State.ApartmentPos then
            TweenTeleport(Configuration.State.ApartmentPos)
        end
        return true
    end
    return false
...
-- ============================================
-- CARI APARTEMEN KOSONG / MILIK SENDIRI
-- ============================================
                ...
                if nameLabel then
                    ...
                    if surfaceGui then
                        ...
                        if textLabel and textLabel.Text == Player.Name then
                            ...
                            Configuration.State.ApartmentPos = aptData.pos
                            return aptData
                        end
                    end
                end
        ...
        if Board then
            ...
            if backboard then
                ...
                if Prompt then
                    ...
                    Configuration.State.Status = "🏠 Beli apartemen..."
        ...
        if DoorLock and Interact then
            ...
            if LockPart and KnobPrompt then
                ...
                if math.abs(LockPart.Rotation.Y) > 5 and math.abs(LockPart.Rotation.Y - 90) > 5 then
                    ...
                    Configuration.State.Status = "🚪 Tutup pintu..."
                ...
                if LockPart.Rotation.X ~= 90 then
                    ...
                    if LockPrompt then
                        ...
                        Configuration.State.Status = "🔒 Kunci pintu..."
                        TweenTeleport(LockPart.Position)
                        task.wait(0.3)
...
local function PourWater()
    ...
    Configuration.State.Status = "💧 Tuang air..."
...
local function AddSugarAndGelatin()
    ...
    Configuration.State.Status = "🍬 Tambah gula..."
    TweenTeleport(stove.Position)
    task.wait(0.3)
...
local function BagMarshmallowAndSell()
    ...
    Configuration.State.Status = "⏳ Masak marshmallow (max 130s)..."
    local waitTime = 0
    repeat 
        task.wait(1) 
        waitTime = waitTime + 1 
    ...
    Configuration.State.Status = "💰 Jual marshmallow..."
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- ============================================
        -- STEP 1: AUTO RESPAWN (LANGSUNG KE APARTEMEN)
        -- ============================================
        Configuration.State.Status = "🔄 Respawn ke apartemen..."
        AutoRespawnToApartment()
        task.wait(1)
        ...
        -- ============================================
        -- STEP 4: AUTO RESPAWN (RESET POSISI)
        -- ============================================
        Configuration.State.Status = "🔄 Respawn reset posisi..."
        AutoRespawn()
        task.wait(1)
        
        -- ============================================
        -- STEP 5: KE PENJUAL MARSHMALLOW
        -- ============================================
        Configuration.State.Status = "🚀 Ke toko..."
        TweenTeleport(Locations.BuyMarsh)
        task.wait(0.3)
        ...
        -- ============================================
        -- STEP 7: TWEEN UNDERGROUND
        -- ============================================
        Configuration.State.Status = "⬇️ Underground..."
        TweenTeleport(Locations.Underground)
        task.wait(0.3)
        
        -- ============================================
        -- STEP 8: KE APARTEMEN (PANCI)
        -- ============================================
        ...
        -- STEP 9: MASAK
        -- ============================================
        Configuration.State.Status = "🍳 Masak..."
        ...
        -- ============================================
        -- STEP 10: TWEEN UNDERGROUND + JUAL
        -- ============================================
        Configuration.State.Status = "💰 Jual..."
