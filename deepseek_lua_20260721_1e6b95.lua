...
-- UI Menggunakan Hyphon Library (sama seperti script Multifarm)
-- ============================================
...
-- ============================================
-- LOAD HYPHON LIBRARY (SAMA SEPERTI SCRIPT MULTIFARM)
-- ============================================
...
-- ============================================
-- UI MENGGUNAKAN HYPHON LIBRARY (SAMA SEPERTI MULTIFARM)
-- ============================================
local Watermark = Library:Watermark("WANZZ Multifarm")
local KeybindList = Library:KeybindList()

local Window = Library:Window({
    Name = "WANZZ Multifarm - South Bronx: The Trenches"
})
...
MainSettingsSection:Toggle({
    ...
    Flag = "Auto Rob Casino",
    Default = Configuration.Main_Settings.AutoRobCasino,
    Callback = function(Value)
        Configuration.Main_Settings.AutoRobCasino = Value
    end,
})
...
MainSettingsSection:Toggle({
    ...
    Default = Configuration.Main_Settings.AutoAntiDeath,
    Callback = function(Value)
...
MainSettingsSection:Toggle({
    Name = "Auto Rejoiner",
    Flag = "Auto Rejoiner",
    Default = Configuration.Main_Settings.AutoRejoiner,
    Callback = function(Value)
...
MainSettingsSection:Toggle({
    ...
    Default = Configuration.Main_Settings.PerformanceSaver,
    Callback = function(Value)
        Configuration.Main_Settings.PerformanceSaver = Value
...
local TargetAmountSlider = GoalSettingsSection:Slider({
    ...
    Min = 0,
    Max = 1750000,
    Default = Configuration.Goal_Settings.TargetAmount,
    Decimals = 1,
    Suffix = " dollars",
    Callback = function(Value)
        Configuration.Goal_Settings.TargetAmount = Value
    end,
})
...
GoalSettingsSection:Textbox({
    ...
    Default = tostring(Configuration.Goal_Settings.TargetAmount),
    Placeholder = "250000",
    Callback = function(Value)
        local Number = tonumber(Value)
        if Number then
            Configuration.Goal_Settings.TargetAmount = Number
...
local StatusLabel = StatisticsSection:Label("📌 Status: Idle")
local RuntimeLabel = StatisticsSection:Label("⏰ Runtime: 00:00:00")
local CashLabel = StatisticsSection:Label("💰 Cash: $0")
...
local TimesRejoinedLabel = StatisticsSection:Label("🔄 Times Rejoined: 0")
local HotChipsLabel = StatisticsSection:Label("🔥 Hot Chips Left: 0")
local CyclesLabel = StatisticsSection:Label("🔄 Cycles: 0")
local CardsSwipedLabel = StatisticsSection:Label("💳 Cards Swiped: 0")

-- ============================================
-- SIDE 2 - WEBHOOK SETTINGS
-- ============================================
...
WebhookSettingsSection:Toggle({
    Name = "Send Webhooks",
    Flag = "Send Webhooks",
    Default = Configuration.Webhook_Settings.SendWebhooks,
    Callback = function(Value)
        Configuration.Webhook_Settings.SendWebhooks = Value
    end,
})
...
WebhookSettingsSection:Slider({
    ...
    Min = 1,
    Max = 10,
    Default = Configuration.Webhook_Settings.WebhookIntervals,
    Decimals = 1,
    Suffix = "m",
    Callback = function(Value)
        Configuration.Webhook_Settings.WebhookIntervals = Value
    end,
})
...
WebhookSettingsSection:Textbox({
    Name = "Webhook Url",
    Flag = "Webhook Url",
    Default = Configuration.Webhook_Settings.WebhookUrl,
    Placeholder = "https://discord.com/api/webhooks/...",
    Callback = function(Value)
        Configuration.Webhook_Settings.WebhookUrl = Value
    end,
})
...
task.spawn(function()
    ...
    while task.wait(1) do
        ...
        Configuration.Statistics.CashMade = GetCurrentCashAmount() - StartCash

        pcall(function()
            StatusLabel:SetText("📌 Status: " .. (Configuration.State.Status or "Idle"))
            RuntimeLabel:SetText("⏰ Runtime: " .. FormatRuntime(Elapsed))
            CashLabel:SetText("💰 Cash: " .. GetCurrentCash())
            ChipsFedLabel:SetText("🍟 Chips Fed: " .. Configuration.Statistics.ChipsFed)
            MarshmallowsSoldLabel:SetText("🧂 Marshmallows Sold: " .. Configuration.Statistics.MarshmallowsSold)
            TimesRejoinedLabel:SetText("🔄 Times Rejoined: " .. Configuration.Statistics.TimesRejoined)
            HotChipsLabel:SetText("🔥 Hot Chips Left: " .. CountHotChips())
            CyclesLabel:SetText("🔄 Cycles: " .. Configuration.Statistics.CyclesCompleted)
            CardsSwipedLabel:SetText("💳 Cards Swiped: " .. Configuration.Statistics.CardsSwiped)
        end)
    end
end)
...
print("UI Menggunakan Hyphon Library (sama seperti script Multifarm)")
