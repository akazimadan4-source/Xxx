...
-- Perbaikan: Siklus 3 tidak stuck, feeding akurat
-- ============================================
...
local function EnsureApartment()
    if Configuration.State.Apartment then return true end
    for i = 1, 5 do
...
-- ============================================
-- MARSHMALLOW DENGAN TIMER (DIPERBAIKI)
-- ============================================
local function PourWater()
    WaitForReady()
    local AptObj = Configuration.State.Apartment
    if not AptObj then
        Configuration.State.Status = "[MARSH] Apartment not set, re-finding..."
        if not StartMarshmallowFarm() then
            return false
        end
    ...
    if not Stove then
        Configuration.State.Status = "[MARSH] Stove not found, re-finding apartment..."
...
-- ============================================
-- COOK POTATO CHIPS (DIPERBAIKI - RESET DAN KELUAR)
-- ============================================
local function CookPotatoChips()
    ...
    -- Reset variabel pot sebelum mencari
    AvailablePot = nil
    PotPrompt = nil
    PotTimer = nil
    ...
    -- KELUAR DARI LABORATORIUM, KEMBALI KE APARTEMEN
    Configuration.State.Status = "[POTATO] Pot found, returning to apartment"
    if Configuration.State.Apartment then
        local AptObj = Configuration.State.Apartment
        local Stove
        if tostring(AptObj):match("Home") then
            Stove = AptObj:FindFirstChild("Cooking Pot")
...
-- ============================================
-- FUNGSI FEED HOMELESS (DIPERBAIKI - SEPERTI INTERAKSI PENJUAL)
-- ============================================
local function FeedHomelessAtCoord(coord)
    ...
    -- Teleport ke koordinat dengan motor
    Configuration.State.Status = "[FEED] Teleporting to " .. tostring(coord)
    DirtBikeTeleport(coord)
    task.wait(1.2)

    -- Cari ProximityPrompt terdekat dengan pendekatan seperti interaksi penjual
    local bestPrompt = nil
    local bestDist = math.huge
    local promptPart = nil

    -- Cari semua prompt di sekitar
        ...
        if obj:IsA("ProximityPrompt") then
            ...
            if part then
                ...
                if dist < 30 and dist < bestDist then
                    -- Periksa apakah bagian dari model yang memiliki Humanoid dan duduk
    ...
    -- Jika tidak ada prompt, coba cari NPC dengan Humanoid duduk
    ...
    -- Konfigurasi prompt seperti interaksi penjual
    pcall(function()
    ...
    -- Teleport lebih dekat ke prompt
    if promptPart then
    ...
    -- Equip Hot Chips dengan retry (seperti equip marshmallow)
    ...
    -- Fire prompt beberapa kali (seperti interaksi penjual)
    ...
    -- Verifikasi sukses dengan mengecek apakah Hot Chips berkurang
    task.wait(0.5)
    ...
    if not remaining then
        ...
        -- Bandingkan jumlah awal dengan sekarang
        local before = CountHotChips() + 1 -- asumsi sebelum diberi
        -- Anggap sukses karena prompt sudah difire
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- ================================
        -- 3. Persiapan potato chips (async)
        -- ================================
        ...
        task.spawn(function()
            ...
            BagPotato()
            MixFlourAndPotato()
            CookPotatoChips()  -- Sekarang akan kembali ke apartemen setelah menemukan pot
            potatoPrepDone = true
        end)
        ...
        -- ================================
        -- 6. Selesaikan marshmallow & chips
        -- ================================
        ...
        -- ================================
        -- RESET variabel pot untuk siklus berikutnya
        -- ================================
        AvailablePot = nil
        PotPrompt = nil
        PotTimer = nil
