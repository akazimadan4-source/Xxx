...
local function AutoRespawn()
    Configuration.State.Status = "🔄 [1] Auto Respawn..."
    local character = Player.Character
    if not character then
        Player.CharacterAdded:Wait()
        return true
...
local function FindAvailableApartment()
    Configuration.State.Status = "🔍 [2] Cari Apartemen..."
...
local function PurchaseMarshmallowIngredients()
    Configuration.State.Status = "📦 [5] Beli Bahan..."
    local Water, Gelatin, SugarBlockBag, EmptyBag = ScavengeInventory()
    if Water >= 1 and Gelatin >= 1 and SugarBlockBag >= 1 and EmptyBag >= 1 then return true end
    ...
    if EmptyBag < 1 then pcall(function() MarshRemote:FireServer("Empty Bag") end) task.wait(0.3) end
    
    return true
end

-- ============================================
-- STEP 6-7: UNDERGROUND + KE APARTEMEN
-- ============================================
local function GoToApartment(aptData)
    Configuration.State.Status = "⬇️ [6] Underground..."
    TweenTeleport(Locations.Underground)
    task.wait(0.3)
    
    Configuration.State.Status = "🏠 [7] Ke Apartemen..."
...
-- ============================================
-- STEP 8: MULAI MEMASAK
-- ============================================
local function CookMarshmallow()
    Configuration.State.Status = "🍳 [8] Mulai Masak..."
    local stove = GetStove()
...
-- ============================================
-- STEP 9: TUNGGU PROSES MASAK
-- ============================================
local function WaitForCook()
    Configuration.State.Status = "⏳ [9] Tunggu Masak (max 130s)..."
    local stove = GetStove()
    if not stove then return false end
    ...
    repeat 
        ...
        if waitTime > 130 then 
            Configuration.State.Status = "❌ Timeout! Ulang masak..."
            return false 
        end 
...
-- ============================================
-- STEP 10: KEMAS MARSHMALLOW
-- ============================================
local function BagMarshmallow()
    Configuration.State.Status = "📦 [10] Kemas Marshmallow..."
    local stove = GetStove()
    if not stove then return false end
...
-- ============================================
-- STEP 11-12: KEMBALI KE PENJUAL & JUAL
-- ============================================
local function SellMarshmallow()
    Configuration.State.Status = "🚀 [11] Ke Penjual..."
    TweenTeleport(Locations.Underground)
    task.wait(0.3)
    TweenTeleport(Locations.BuyMarsh)
    task.wait(0.3)
    
    Configuration.State.Status = "💰 [12] Jual Semua..."
...
-- ============================================
-- MAIN AUTOFARM CONTROLLER (FULL LOOP)
-- ============================================
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- STEP 5: AUTO RESPAWN (reset posisi)
        Configuration.State.Status = "🔄 [5] Auto Respawn..."
        AutoRespawn()
        task.wait(0.5)
        
        -- STEP 6-7: KE PENJUAL & BELI BAHAN
        Configuration.State.Status = "🚀 Ke Penjual..."
        TweenTeleport(Locations.BuyMarsh)
        task.wait(0.3)
        PurchaseMarshmallowIngredients()
        
        -- STEP 8-9: UNDERGROUND + KE APARTEMEN
        GoToApartment(aptData)
        
        -- STEP 10: MULAI MEMASAK
        local cookSuccess = CookMarshmallow()
        if not cookSuccess then
            Configuration.State.Status = "❌ Gagal masak, ulang dari awal..."
            AutoRespawn()
            task.wait(2)
            continue
        end
        
        -- STEP 11: TUNGGU PROSES MASAK
        local waitSuccess = WaitForCook()
        if not waitSuccess then
            Configuration.State.Status = "❌ Timeout, ulang dari awal..."
            AutoRespawn()
            task.wait(2)
            continue
        end
        
        -- STEP 12: KEMAS MARSHMALLOW
        BagMarshmallow()
        
        -- STEP 13-14: JUAL MARSHMALLOW
        SellMarshmallow()
        
        Configuration.State.Status = "✅ Selesai 1 cycle! Ulang dari Step 1..."
        task.wait(1)
    end
...
StepLabel.Size = UDim2.new(1, -20, 0, 25)
StepLabel.Position = UDim2.new(0, 10, 0, 48)
...
StatusLabel.Position = UDim2.new(0, 10, 0, 73)
