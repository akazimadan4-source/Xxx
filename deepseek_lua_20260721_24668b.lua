...
-- Perbaikan logika pot + perlambatan teleport
-- ============================================
...
local function DirtBikeTeleportStealth(TargetPosition)
    ...
    if not h.SeatPart then
        ...
        task.wait(0.5)  -- PERUBAHAN: jeda lebih lama
    end
    ...
    if distance > 30 then
        local steps = math.min(math.floor(distance / 15), 6) -- lebih sedikit step
        for i = 1, steps do
            ...
            task.wait(0.2) -- jeda lebih lama
        end
    end
    pcall(function()
        Vehicle:PivotTo(CFrame.new(TargetPosition))
        task.wait(0.5) -- jeda lebih lama setelah teleport
        for _, part in pairs(Vehicle:GetDescendants()) do
            if part:IsA("BasePart") then part.Anchored = false end
        end
        task.wait(0.3)
        ...
        task.wait(0.5) -- jeda lebih lama
        for _, part in pairs(Vehicle:GetDescendants()) do
            if part:IsA("BasePart") then part.Anchored = false end
        end
    end)
    RepairBike()
    task.wait(0.3) -- tambah jeda akhir
    return true
end
