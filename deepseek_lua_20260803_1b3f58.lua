-- ============================================
-- LENGER MULTI FARM (MODIFIED)
-- Perbaikan: Spawn bike, teleport, potato workflow menggunakan Multi Farm.
-- Interaksi Homeless dikembalikan ke versi asli (DeepSeek).
-- Fix: RPC loading dengan timeout untuk menghindari infinite yield.
-- ============================================
...
-- ============================================
-- RPC LOADING DENGAN TIMEOUT (FIX INFINITE YIELD)
-- ============================================
local function getRPC()
    local RemoteEvents = ReplicatedStorage:FindFirstChild("RemoteEvents")
    if not RemoteEvents then
        -- Tunggu sebentar sampai RemoteEvents muncul
        for i = 1, 10 do
            task.wait(0.5)
            RemoteEvents = ReplicatedStorage:FindFirstChild("RemoteEvents")
            if RemoteEvents then break end
        end
        if not RemoteEvents then
            error("RemoteEvents not found after 5 seconds")
        end
    end
    ...
    if not RPC then
        for i = 1, 10 do
            task.wait(0.5)
            RPC = RemoteEvents:FindFirstChild("RPC")
            if RPC then break end
        end
        if not RPC then
            error("RPC not found after 5 seconds")
        end
    end
    return RPC
end
...
-- ============================================
-- DAFTAR KOORDINAT HOMELESS (12 TITIK) - asli
-- ============================================
...
-- ============================================
-- FUNGSI SPAWN & SIT BIKE (VERSI MULTI FARM)
-- ============================================
...
-- ============================================
-- DIRTBIKE TELEPORT (VERSI MULTI FARM - CEPAT)
-- ============================================
...
-- ============================================
-- LOKASI (dengan koordinat Multi Farm untuk Potato)
-- ============================================
local Locations = {
    ...
    BuyPotato     = Vector3.new(-759.920, -0.025, -195.870),  -- dari Multi Farm
    Healing       = Vector3.new(-769.000,  6.000,  654.000),
    Clipboard     = Vector3.new(-479.230,  5.342, -433.270),  -- dari Multi Farm
    PotatoCutter  = Vector3.new(-456.320,  1.870, -466.840),  -- dari Multi Farm
    PlasticBagLab = Vector3.new(-456.280,  1.654, -472.670),  -- dari Multi Farm
    FlourBowl     = Vector3.new(-494.640,  1.579, -518.580),  -- dari Multi Farm
...
-- ============================================
-- MARSHMALLOW DENGAN TIMER (tetap)
-- ============================================
...
-- ============================================
-- POTATO CHIPS FUNCTIONS (VERSI MULTI FARM)
-- ============================================
...
-- ============================================
-- FEED HOMELESS (VERSI ASLI DEEPSEEK)
-- ============================================
...
-- ============================================
-- CLAIM POTATO CHIPS & FEED HOMELESS (DEEPSEEK STYLE)
-- ============================================
local function ClaimPotatoChipsAndSell()
    ...
    -- Feeding Homeless menggunakan metode asli DeepSeek
    local fedCount = 0
...
-- ============================================
-- MAIN AUTOFARM CONTROLLER (tetap)
-- ============================================
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- Potato preparation (sequential, tanpa task.spawn)
...
-- ============================================
-- UI (TIDAK DIUBAH - SAMA PERSIS DENGAN DEEPSEEK)
-- ============================================
