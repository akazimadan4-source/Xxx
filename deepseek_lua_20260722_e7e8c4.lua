...
-- ============================================
-- WANZZ PROJECT - BFG MULTIFARM (FIX BUG v8)
-- Perbaikan berdasarkan analisis:
--   1. Deteksi notifikasi pot menggunakan find()
--   2. Controller menangani hasil CookPotatoChips()
--   3. ChipsCooking dicek di awal CookPotatoChips()
--   4. Reset state setelah claim
--   5. Sugar/Gelatin hanya jalan jika ChipsCooking true
-- ============================================
...
local function PurchaseFakeID()
    ...
    repeat WaitForReady() DirtBikeTeleport(Locations.FakeID) task.wait(0.15)
...
local function ApplyForCard()
    ...
    repeat DirtBikeTeleport(Locations.ApplyForCard) task.wait(0.15)
...
-- ============================================
-- MODIFIKASI: AddSugarAndGelatin dengan pengecekan ChipsCooking (PERBAIKAN #5)
-- ============================================
local function AddSugarAndGelatin()
    WaitForReady()
    -- PERBAIKAN #5: Cek apakah chips sudah dimasak
...
-- ============================================
-- COOK POTATO CHIPS - PERBAIKAN #1 dan #3
-- ============================================
local function CookPotatoChips()
    WaitForReady()
    
    -- PERBAIKAN #3: Jika sudah ada pot yang sedang memasak, jangan cari lagi
        ...
        if prompt then
            ...
            for attempt = 1, 5 do
                ...
                if notif and notif.TextTransparency == 0 then
                    ...
                    -- PERBAIKAN #1: deteksi notifikasi dengan find()
                    if text and text:find("retrieve your product") then
                        AvailablePot = pot
...
-- ============================================
-- CLAIM & FEED HOMELESS - PERBAIKAN #4 (RESET STATE)
-- ============================================
local function ClaimPotatoChipsAndSell()
    ...
    if not AvailablePot or not PotPrompt then
        ...
        -- PERBAIKAN #4: reset di sini juga
    ...
    -- PERBAIKAN #4: RESET STATE SETELAH CLAIM (baik sukses atau gagal)
    AvailablePot = nil
    PotPrompt = nil
    PotTimer = nil
    Configuration.State.ChipsCooking = false
...
-- ============================================
-- MAIN AUTOFARM CONTROLLER - PERBAIKAN #2
-- ============================================
...
local function MainAutofarmController()
    ...
    while Configuration.Main_Settings.Autofarming do
        ...
        -- 2. Beli bahan marshmallow (Water, Sugar, Gelatin)
        ...
        -- 4. Siapkan chips (beli bahan, potong, campur)
        ...
        -- 5. Masukkan chips ke pot (hanya jika belum cooking)
        -- PERBAIKAN #2: tangani hasil CookPotatoChips
        local Success = CookPotatoChips()
        if not Success then
            Configuration.State.Status = "[POTATO] Failed to get pot, retrying cycle"
            task.wait(1)
            continue
        end
        Configuration.State.CurrentStep = "CHIPS_IN_POT"

        -- 6. Tambahkan sugar & gelatin (hanya jika chips sudah di pot)
        AddSugarAndGelatin()
        Configuration.State.CurrentStep = "SUGAR_GELATIN_ADDED"

        -- 7. Card Scam: beli fake ID & apply
        if Configuration.Main_Settings.EnableCardScam then
            PurchaseFakeID()
            ApplyForCard()
            -- Tunggu notifikasi approval (maks 40 detik)
        ...
        -- 8. Tunggu marshmallow masak (40 detik)
        for i = 40, 1, -1 do
            if not Configuration.Main_Settings.Autofarming then break end
        ...
        -- 9. Kemas dan jual marshmallow
        BagMarshmallowAndSell()
        Configuration.State.CurrentStep = "MARSH_SOLD"

        -- 10. Claim chips, convert ke hot chips, dan feed homeless
        --     Fungsi ini akan mereset ChipsCooking
        if Configuration.State.ChipsCooking then
        ...
        Configuration.State.CurrentStep = "CYCLE_COMPLETE"
    end
