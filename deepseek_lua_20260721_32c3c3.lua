-- ============================================
-- WANZZ MULTIFARM - DELTA FIX (Manual UI)
-- UI tanpa Hyphon, kompatibel dengan Delta
-- ============================================
...
-- ============================================
-- LOAD LIBRARY YANG SUPPORT DELTA
-- ============================================
-- Pake library alternatif yang lebih stabil
local UIlib = loadstring(game:HttpGet("https://raw.githubusercontent.com/3dsboy08/cat/main/main.lua"))()

local Window = UIlib:CreateWindow("WANZZ MULTIFARM")
local MainTab = Window:Tab("Main", "rbxassetid://6031091071")
...
-- UI SECTION
-- ============================================
MainTab:Section("Main Settings")

MainTab:Toggle("Autofarming", function(v)
...
MainTab:Toggle("Auto Rob Casino", function(v)
    Configuration.Main_Settings.AutoRobCasino = v
end)

MainTab:Toggle("Auto Anti Death", function(v)
    Configuration.Main_Settings.AutoAntiDeath = v
end)

MainTab:Toggle("Auto Rejoiner", function(v)
    Configuration.Main_Settings.AutoRejoiner = v
end)

MainTab:Button("Purchase DirtBike ($35000)", function()
    pcall(function()
        RPC:FireServer(buffer.fromstring("\001"), "Purchase", "DirtBike")
    end)
end)

MainTab:Button("Rejoin Game", function()
    TeleportService:Teleport(10179538382)
end)

-- Statistics Section
MainTab:Section("Statistics")

local statusLabel = MainTab:Label("📌 Status: Idle")
local runtimeLabel = MainTab:Label("⏰ Runtime: 00:00:00")
local cashLabel = MainTab:Label("💰 Cash: $0")
local chipsLabel = MainTab:Label("🍟 Chips Fed: 0")
local marshLabel = MainTab:Label("🧂 Marshmallows Sold: 0")
local rejoinLabel = MainTab:Label("🔄 Times Rejoined: 0")
local hotChipsLabel = MainTab:Label("🔥 Hot Chips Left: 0")
local cyclesLabel = MainTab:Label("🔄 Cycles: 0")
local cardsLabel = MainTab:Label("💳 Cards Swiped: 0")
...
task.spawn(function()
    ...
    while task.wait(1) do
        ...
        pcall(function()
            ...
            cashLabel:SetText("💰 Cash: $" .. GetCommaValue(GetCurrentCashAmount()))
            ...
            cardsLabel:SetText("💳 Cards Swiped: " .. Configuration.Statistics.CardsSwiped)
        end)
    end
end)

-- ============================================
-- AUTOFARM CONTROLLER (SAMA SEPERTI SEBELUMNYA)
-- ============================================
...
print("WANZZ MULTIFARM - DELTA FIX LOADED!")
print("UI Menggunakan Library Alternatif yang Support Delta")
print("Menu: Klik icon di layar atau swipe")