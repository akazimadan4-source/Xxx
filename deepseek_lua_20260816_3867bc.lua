-- Deobfuscated and Cleaned Version with Enhanced Teleport System
-- Original source: 1786781816448_pr_2260dd0c_191HP.lua
-- Modified with anti-detection platform-based teleport
...
-- Teleport position presets (with Y coordinate for surface)
local TELEPORT_POSITIONS = {
    {pos = CFrame.new(770.992, 3.71, 433.75), name = "NPC 1"},
    {pos = CFrame.new(510.061, 4.476, 600.548), name = "NPC 2"},
    {pos = CFrame.new(1137.992, 9.932, 449.753), name = "Location 3"},
    {pos = CFrame.new(1139.174, 9.932, 420.556), name = "Location 4"},
    {pos = CFrame.new(984.856, 9.932, 247.28), name = "Location 5"},
    {pos = CFrame.new(988.311, 9.932, 221.664), name = "Location 6"},
    {pos = CFrame.new(923.954, 9.932, 42.202), name = "Location 7"},
    {pos = CFrame.new(895.721, 9.932, 41.928), name = "Location 8"},
    {pos = CFrame.new(1166.33, 3.36, -29.77), name = "Location 9"},
    {pos = CFrame.new(1065.19, 28.47, 420.76), name = "Location 10"},
    {pos = CFrame.new(1202.3, 3.71, -220.91), name = "Location 11"},
    {pos = CFrame.new(1179.72, 3.71, -230.21), name = "Location 12"},
    {pos = CFrame.new(1202.31, 3.71, -182.55), name = "Location 13"}
}
...
-- ============================================
-- Enhanced Teleport System with Anti-Detection
-- ============================================

-- Create a platform underground to move on
...
local function goUnderground()
    ...
    pcall(function()
        hrp.Anchored = true
        hrp.CFrame = CFrame.new(hrp.Position.X, platformY + 0.5 + surfaceOffset, hrp.Position.Z)
        hrp.Anchored = false
    end)
    task.wait(0.05)
end

-- Surface (rise above ground)
local function surface()
    ...
    pcall(function()
        hrp.Anchored = true
        hrp.CFrame = CFrame.new(hrp.Position.X, platformY + 11 + surfaceOffset, hrp.Position.Z)
        hrp.Anchored = false
    end)
    task.wait(0.3)
    destroyPlatform()
...
local function teleportSmooth(targetPos, yBase, onProgress)
    local char = LocalPlayer.Character
    if not char then return end
    local hrp = char:FindFirstChild("HumanoidRootPart")
    ...
    -- Ensure we are at the right height
    ...
    local moveConn = RunService.Heartbeat:Connect(function(dt)
        ...
        local moveSpeed = teleportSpeed
        if slowNearDest then
            moveSpeed = teleportSpeed * math.clamp(distRemaining / slowdownDist, slowdownMin, 1)
        end
...
-- Perform full teleport sequence
local function performTeleport(targetPos, callback)
    if isMoving then
        return
    end
    ...
    isMoving = true
    task.spawn(function()
        -- Create platform underground
        local yBase = heightOffset
        createPlatform(yBase)
        ...
        teleportSmooth(Vector3.new(targetX, 0, targetZ), yBase, function(pct)
            if callback then
                callback(pct)
            end
        end)
...
-- Teleport to position (compatible with old API)
local function teleportToPosition(pos)
    ...
    -- Use new system if it's a CFrame with X/Y/Z
    local x, y, z = pos.X, pos.Y, pos.Z
    performTeleport(Vector3.new(x, 0, z), nil)
end

-- Legacy teleport function (for vehicle or direct)
local function teleportDirect(pos)
    local char = LocalPlayer.Character
    if not char then return end
    local hrp = char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
...
-- Teleport vehicle (unchanged but uses direct method)
...
local function fullyLoop()
    while fullyLoopRunning do
        -- Teleport to NPC using new system
        performTeleport(Vector3.new(TELEPORT_POSITIONS[1].pos.X, 0, TELEPORT_POSITIONS[1].pos.Z), nil)
        task.wait(1)
        ...
        -- Return to apartment using new system
        performTeleport(Vector3.new(APARTMENT_CFRAME.X, 0, APARTMENT_CFRAME.Z), nil)
        task.wait(1)
...
-- ============================================
-- UI Creation (unchanged but references updated)
-- ============================================
...
local function createUI()
    ...
    function createButton(parent, text, color, order)
        ...
        button.MouseLeave:Connect(function()
            TweenService:Create(button, TweenInfo.new(0.12), {BackgroundColor3 = color or COLORS.accentDim}):Play()
        end)
        return button
    end

    -- Build AUTO Tab (same as before)
    ...
    for i, data in ipairs(TELEPORT_POSITIONS) do
        local btn = createButton(tpTab, data.name or "Location " .. i, COLORS.accentDim, i + 1)
        btn.MouseButton1Click:Connect(function()
            performTeleport(Vector3.new(data.pos.X, 0, data.pos.Z), nil)
        end)
    end

    -- Build MS POT Tab (placeholder)
    local msPotTab = tabFrames["MS POT"]
    createSection(msPotTab, "MS POT SETTINGS", 1)

    -- Build BUY Tab (placeholder)
    ...
    antiAfkButton.MouseButton1Click:Connect(function()
        ...
        antiAfkButton.Text = antiAfkActive and "ON" or "OFF"
        antiAfkButton.BackgroundColor3 = antiAfkActive and COLORS.green or COLORS.red
    end)
    ...
    blinkButton.MouseButton1Click:Connect(function()
        ...
        blinkButton.Text = blinkEnabled and "ON" or "OFF"
        blinkButton.BackgroundColor3 = blinkEnabled and COLORS.green or COLORS.red
    end)
    ...
    deleteButton.MouseButton1Click:Connect(function()
        ...
        if result then
            ...
            if obj then
                table.insert(deletedObjects, {object = obj:Clone(), parent = obj.Parent})
                obj:Destroy()
            end
        end
        task.wait(0.3)
